...
> Nên app vẫn dùng thư viện `openai` như cũ — ta chỉ trỏ
> `baseURL` về server của bạn và đặt một API key tuỳ ý.
>
> Lưu ý: Knowledge Base của app **không dùng embedding** — tìm kiếm tài liệu
> chạy bằng PostgreSQL Full Text Search (tsvector + GIN + ts_rank) ngay trong
> Supabase, nên server này chỉ cần phục vụ model chat/vision, không cần chạy
> thêm model embedding nào.
...
- Cổng public: bạn nói `103.73.232.112:3333` và `:4444` đã public.
  → Ta sẽ chạy model chat ở **4444** (cổng `3333` không còn cần dùng cho embedding).
...
## 7. Knowledge Base — KHÔNG cần embedding server

App **không dùng embedding ở bất kỳ đâu**. Toàn bộ tính năng Knowledge Base
(tải PDF lên, hỏi-đáp dựa trên tài liệu) chạy bằng **PostgreSQL Full Text
Search** (`tsvector` + chỉ mục **GIN** + `ts_rank`) ngay trong Supabase —
xem `migrations/fulltext_search.sql`. Vì vậy:

- Không cần chạy thêm model embedding (BGE-M3 hay bất kỳ model nào khác).
- Không cần `EMBEDDING_BASE_URL` / `EMBEDDING_API_KEY` / `EMBEDDING_MODEL`.
- Không cần pgvector hay bất kỳ vector database nào.
- Chỉ cần chạy migration + (tuỳ chọn) seed dữ liệu nền có sẵn:
  ```bash
  # 1) Trên Supabase SQL Editor: chạy nội dung migrations/fulltext_search.sql
  # 2) Ở máy/app (tuỳ chọn — nạp kho kiến thức bệnh lý có sẵn vào Postgres):
  node scripts/seed-base-knowledge.mjs
  ```

Chi tiết kiến trúc: xem `docs/KNOWLEDGE-BASE-FULLTEXT-SEARCH.md`.
