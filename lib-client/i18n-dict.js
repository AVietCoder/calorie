// lib-client/i18n-dict.js — VI/EN dictionary, ported verbatim from the old
// public/i18n.js DICT object (same keys/values) so every page keeps its
// existing copy after the React rewrite.
export const DICT = {
  vi: {
    "common.lang": "Ngôn ngữ",
    "common.save": "Lưu thay đổi",
    "common.cancel": "Hủy",
    "common.close": "Đóng",
    "common.add": "Thêm",
    "common.delete": "Xóa",
    "common.loading": "Đang tải...",
    "common.processing": "Đang xử lý...",
    "common.saving": "Đang lưu...",
    "common.creating": "Đang tạo...",
    "common.uploading": "Đang tải lên...",
    "common.sending": "Đang gửi...",
    "common.deleting": "Đang xóa...",
    "common.logout": "Đăng xuất",
    "common.logout_hint": "Nhấn để đăng xuất",
    "common.kcal": "kcal",
    "common.today": "Hôm nay",
    "nav.ai": "AI",
    "nav.diet": "DIET",
    "nav.plan": "PLAN",
    "nav.family": "GIA ĐÌNH",
    "nav.profile": "PROFILE",
    "nav.guide": "GUIDE",
    "land.signin": "Đăng nhập",
    "land.signup": "Đăng ký",
    "land.badge": "Trí tuệ nhân tạo thế hệ mới",
    "land.title1": "Kiểm soát Calorie",
    "land.title2": "Dễ dàng hơn bao giờ hết.",
    "land.desc": "Chỉ cần gửi ảnh hoặc mô tả món ăn, hệ thống sẽ phân tích dinh dưỡng và đưa ra gợi ý phù hợp, đồng thời điều chỉnh kế hoạch ăn uống theo mục tiêu và thói quen thực tế của bạn.",
    "land.cta": "Bắt đầu ngay",
    "land.footer": "© 2026 Dr.Fit. Phân tích dinh dưỡng thông minh.",
    "chat.greeting": "Chào bạn! Hãy gửi tin nhắn hoặc ảnh món ăn, tôi sẽ phân tích giúp bạn.",
    "chat.placeholder": "Hỏi tôi liên quan về dinh dưỡng...",
    "chat.total_energy": "Tổng năng lượng",
    "chat.info_update": "Thông tin sẽ cập nhật sau khi phân tích.",
    "chat.protein": "Protein",
    "chat.fat": "Chất béo",
    "chat.carbs": "Carbs",
    "chat.fiber": "Chất xơ",
    "chat.sugar": "Đường",
    "chat.sodium": "Natri",
    "chat.upload_hint": "Tải ảnh món ăn",
    "chat.send": "Gửi",
    "chat.analyze_image": "Phân tích hình ảnh này",
    "chat.analyzing": "AI đang phân tích...",
    "chat.loading_history": "Đang tải lịch sử...",
    "chat.pasted_image": "Đã dán ảnh! Nhấn Gửi để phân tích.",
    "chat.pasted_label": "Ảnh vừa dán (Ctrl+V)",
    "chat.remove_image": "Xoá ảnh",
    "chat.jpg_only": "Chỉ được gửi ảnh JPG!",
    "chat.sent_image": "[Đã gửi ảnh]",
    "chat.voice_unsupported": "Trình duyệt của bạn không hỗ trợ nhận giọng nói.",
    "chat.voice_denied": "Vui lòng cấp quyền microphone.",
    "chat.voice_nospeech": "Không nghe thấy gì, thử lại nhé.",
    "chat.voice_neterr": "Lỗi mạng khi nhận giọng nói.",
    "common.please_login": "Vui lòng đăng nhập!",
    "common.logging_out": "Đang đăng xuất...",
    "common.error_generic": "Có lỗi đang xảy ra!",
    "common.session_expired": "Phiên đăng nhập hết hạn. Vui lòng đăng nhập lại.",
    "meal.confirm_title": "Xác nhận bữa ăn của bạn",
    "meal.choose_time": "Chọn buổi ăn",
    "meal.breakfast": "Sáng",
    "meal.lunch": "Trưa",
    "meal.dinner": "Tối",
    "meal.snack": "Bữa phụ",
    "meal.when": "Thời điểm",
    "meal.today": "Hôm nay",
    "meal.other_day": "Ngày khác",
    "meal.confirm": "Xác nhận",
    "meal.cancel": "Hủy",
    "meal.pick_time": "Hãy chọn buổi ăn",
    "meal.cancelled": "Đã hủy. Bạn có thể nhập món khác.",
    "meal.updating": "Đang cập nhật thực đơn...",
    "meal.logged": "Đã ghi lại bữa ăn của bạn!",
    "meal.update_fail": "Mình chưa cập nhật được, bạn thử lại nhé.",
    "meal.updated_toast": "Đã cập nhật thời khóa biểu & thống kê!",
    "meal.conn_err": "Lỗi kết nối server",
    "meal.confirm_sent": "Xác nhận: Ăn vào buổi {meal}, {day}",
    "sch.hero_title": "Lộ trình thực đơn của bạn",
    "sch.hero_desc": "AI lên lịch 7 ngày dựa trên mục tiêu, khẩu vị và cường độ vận động",
    "sch.coach_title": "HLV Dinh Dưỡng AI",
    "sch.coach_desc": "Trao đổi với AI để cập nhật thực đơn và theo dõi bữa ăn.",
    "sch.week_title": "Lộ trình thực đơn 7 ngày",
    "sch.meal": "Bữa",
    "sch.breakfast": "Sáng",
    "sch.lunch": "Trưa",
    "sch.dinner": "Tối",
    "sch.snack": "Phụ",
    "sch.eaten": "Đã ăn",
    "sch.eaten_q": "Đã ăn bữa này chưa?",
    "sch.mark_eaten": "Đánh dấu đã ăn",
    "sch.will_eat": "Tôi sẽ ăn món này",
    "sch.alt_ph": "Bạn muốn ăn món gì khác?",
    "sch.confirm_change": "Xác nhận đổi món",
    "sch.alt_hint": "Nhập tên món bạn muốn đổi rồi bấm xác nhận",
    "sch.find_near": "Tìm quán gần đây",
    "sch.ask_ai": "Hỏi HLV AI",
    "sch.nutrition_struct": "Cơ cấu dinh dưỡng",
    "today.title": "Hôm nay bạn đã nạp",
    "today.subtitle": "Tổng năng lượng & dinh dưỡng đã ăn trong ngày",
    "today.consumed": "Đã nạp",
    "today.target": "Mục tiêu",
    "today.remaining": "Còn lại",
    "today.over": "Vượt mức",
    "today.no_meal": "Bạn chưa đánh dấu bữa nào hôm nay. Tick \"Đã ăn\" ở từng bữa để theo dõi.",
    "extra.add_btn": "Thêm món ăn ngoài thực đơn",
    "extra.title": "Thêm món ăn thêm",
    "extra.desc": "Ăn vặt, trái cây, đồ uống… ngoài thực đơn? Thêm vào đây để tính vào tổng hôm nay.",
    "extra.name": "Tên món",
    "extra.name_ph": "VD: Táo, sữa chua, trà sữa...",
    "extra.kcal": "Năng lượng (kcal)",
    "extra.kcal_ph": "VD: 95",
    "extra.estimate": "Tự động tính bằng AI",
    "extra.estimating": "AI đang ước tính...",
    "extra.low_conf": "Giá trị chỉ mang tính ước lượng.",
    "extra.detected": "Nhận diện",
    "extra.filled_note": "Đã điền vào form — nhấn <b>Thêm</b> để lưu",
    "extra.server_err": "Lỗi server ({status}). Thử lại nhé!",
    "extra.added": "Đã thêm vào hôm nay!",
    "extra.list_title": "Món thêm hôm nay",
    "extra.row": "Món thêm",
    "extra.need_name": "Vui lòng nhập tên món",
    "extra.optional_macro": "Dinh dưỡng (tùy chọn)",
    "extra.upload_photo": "Tải ảnh món ăn",
    "extra.analyzing_photo": "AI đang phân tích ảnh...",
    "extra.photo_done": "Đã phân tích ảnh! Kiểm tra lại số liệu nhé.",
    "extra.photo_fail": "Không phân tích được ảnh",
    "extra.photo_net_err": "Không phân tích được ảnh. Kiểm tra kết nối mạng nhé!",
    "extra.not_food": "Ảnh không giống món ăn. Hãy thử ảnh khác.",
    "extra.photo_hint": "hoặc chụp/tải ảnh để AI tự nhận diện",
    "rem.title": "Nhắc nhở",
    "rem.subtitle": "Nhắc uống thuốc & ăn đúng giờ",
    "rem.tab_meal": "Bữa ăn",
    "rem.tab_med": "Uống thuốc",
    "rem.add": "Thêm nhắc nhở",
    "rem.label": "Nội dung nhắc",
    "rem.label_meal_ph": "VD: Ăn sáng",
    "rem.label_med_ph": "VD: Uống vitamin D",
    "rem.time": "Giờ nhắc",
    "rem.repeat": "Lặp lại hằng ngày",
    "rem.none": "Chưa có nhắc nhở nào.",
    "rem.enable_notif": "Bật thông báo trình duyệt",
    "rem.notif_blocked": "Trình duyệt đã chặn thông báo. Hãy bật lại trong cài đặt.",
    "rem.notif_on": "Đã bật thông báo!",
    "rem.saved": "Đã lưu nhắc nhở",
    "rem.deleted": "Đã xóa nhắc nhở",
    "rem.fire_meal": "🍽️ Đến giờ ăn",
    "rem.fire_med": "💊 Đến giờ uống thuốc",
    "rem.need_time": "Vui lòng chọn giờ nhắc",
    "rem.open": "Nhắc nhở",
    "rem.alarm_now": "BÂY GIỜ",
    "rem.alarm_dismiss": "Đã hiểu",
    "rem.alarm_default_med": "Đã đến giờ uống thuốc của bạn.",
    "rem.alarm_default_meal": "Đã đến giờ ăn của bạn.",
    "setup.body_title": "Chỉ số cơ thể",
    "setup.body_desc": "Để tính toán BMR và TDEE chính xác cho bạn.",
    "setup.gender": "Giới tính",
    "setup.male": "Nam",
    "setup.female": "Nữ",
    "setup.birth": "Năm sinh",
    "setup.height": "Chiều cao (cm)",
    "setup.weight": "Cân nặng (kg)",
    "setup.goal_title": "Mục tiêu của bạn",
    "setup.goal_desc": "Bạn có thể chọn nhiều mục tiêu cùng lúc.",
    "setup.goal_lose": "Giảm cân",
    "setup.goal_maintain": "Giữ cân",
    "setup.goal_gain": "Tăng cân",
    "setup.goal_muscle": "Tăng cơ",
    "setup.goal_disease": "Cải thiện / hỗ trợ điều trị bệnh",
    "setup.disease_label": "Bệnh / tình trạng sức khỏe",
    "setup.disease_pick": "-- Chọn bệnh --",
    "setup.disease_other_ph": "Nhập tên bệnh...",
    "setup.target_weight": "Cân nặng mục tiêu",
    "setup.deadline": "Deadline",
    "setup.speed": "Tốc độ mong muốn",
    "setup.speed_safe": "An toàn (0.5kg/tuần)",
    "setup.speed_normal": "Vừa phải (0.7kg/tuần)",
    "setup.speed_fast": "Nhanh (1kg/tuần)",
    "setup.habit_title": "Thói quen & Linh hoạt",
    "setup.habit_desc": "Tối ưu hóa calo theo lịch trình của bạn.",
    "setup.activity": "Tần suất vận động",
    "setup.act_1": "Ít vận động (Văn phòng)",
    "setup.act_2": "Nhẹ (1-2 buổi tập/tuần)",
    "setup.act_3": "Vừa (3-5 buổi tập/tuần)",
    "setup.act_4": "Nặng (6-7 buổi tập/tuần)",
    "setup.cheat": "Ngày ăn nhiều (Cheat day)?",
    "setup.cheat_ph": "Ví dụ: Thứ 7 và Chủ Nhật",
    "setup.snack_q": "Bạn có hay ăn vặt không?",
    "setup.snack_no": "Không bao giờ",
    "setup.snack_sometimes": "Thỉnh thoảng",
    "setup.snack_often": "Thường xuyên",
    "setup.deep_title": "Cá nhân hóa sâu",
    "setup.deep_desc": "Bước cuối cùng để hoàn tất lộ trình.",
    "setup.allergies": "Dị ứng / Thực phẩm không ăn được",
    "setup.allergies_ph": "Ví dụ: Không ăn hành, dị ứng hải sản...",
    "setup.focus_q": "Bạn muốn tập trung vào chất nào?",
    "setup.focus_balanced": "Cân bằng",
    "setup.focus_protein": "Nhiều Đạm (Protein)",
    "setup.focus_lowcarb": "Ít Tinh bột (Low carb)",
    "setup.reason": "Lý do bắt đầu?",
    "setup.reason_ph": "Cải thiện sức khỏe...",
    "setup.prev": "Quay lại",
    "setup.next": "Tiếp tục",
    "setup.finish": "Hoàn tất lộ trình",
    "setup.extend_banner": "Chặng đường cũ đã hoàn thành. Hãy cập nhật cân nặng mới và đặt deadline mới nhé!",
    "setup.pick_goal": "Vui lòng chọn ít nhất một mục tiêu.",
    "diet.today_route": "Lộ trình hôm nay",
    "diet.daily_target": "Mục tiêu nạp mỗi ngày",
    "diet.weight_now": "Cân nặng hiện tại",
    "diet.target_to": "Mục tiêu hướng đến",
    "diet.deadline": "Thời hạn (Deadline)",
    "diet.nutri_title": "Phân tích dinh dưỡng",
    "diet.nutri_sub": "Tỉ lệ macro & tiến độ cân nặng theo lộ trình",
    "diet.macro_ratio": "Tỉ lệ Macro đề xuất",
    "diet.weight_progress": "Tiến độ cân nặng",
    "diet.cal_per_day": "Calo theo từng ngày trong tuần",
    "auth.signin_title": "Chào mừng trở lại",
    "auth.signin_sub": "Tiếp tục theo dõi sức khỏe của bạn",
    "auth.username": "Tên đăng nhập",
    "auth.username_ph_signin": "Nhập username",
    "auth.password": "Mật khẩu",
    "auth.signin_btn": "Đăng nhập",
    "auth.no_account": "Chưa có tài khoản?",
    "auth.signup_now": "Đăng ký ngay",
    "auth.signup_title": "Tạo tài khoản mới",
    "auth.signup_sub": "Bắt đầu hành trình dinh dưỡng của bạn",
    "auth.username_ph_signup": "Ví dụ: nva123",
    "auth.birth": "Năm sinh",
    "auth.weight": "Cân nặng (kg)",
    "auth.height": "Chiều cao (cm)",
    "auth.signup_btn": "Đăng ký ngay",
    "auth.have_account": "Đã có tài khoản?",
    "auth.signin_link": "Đăng nhập",
    "diet.personalized": "Cá nhân hoá",
    "diet.kcal_per_day": "kcal / ngày",
    "diet.disease_eyebrow": "Bệnh lý cần lưu ý",
    "diet.disease_loading": "Đang tải thông tin sức khoẻ...",
    "diet.disease_loading_desc": "Hệ thống sẽ cá nhân hoá thực đơn dựa trên tình trạng sức khoẻ của bạn.",
    "diet.disease_none_title": "Không có bệnh nền",
    "diet.disease_none_desc": "Bạn chưa khai báo bệnh lý nào. Thực đơn sẽ tối ưu cho mục tiêu cân nặng & năng lượng.",
    "diet.disease_one_title": "Lưu ý chế độ ăn cho tình trạng sức khoẻ",
    "diet.disease_many_title": "Bạn đang có {n} tình trạng cần lưu ý",
    "diet.disease_warn_desc": "Vui lòng chú ý lựa chọn thực phẩm phù hợp. Hệ thống sẽ ưu tiên cảnh báo món ăn không tốt cho các bệnh lý dưới đây.",
    "chart.protein": "Protein",
    "chart.carbs": "Carbs",
    "chart.fats": "Chất béo",
    "chart.weight_kg": "Cân nặng (kg)",
    "chart.target": "Mục tiêu",
    "chart.start": "Bắt đầu",
    "chart.current": "Hiện tại",
    "chart.week_n": "Tuần {n}",
    "chart.cal_intake_est": "Calo nạp (ước tính)",
    "chart.bmr_basal": "BMR (cơ bản)",
    "chart.activity": "Vận động",
    "chart.bmr": "BMR",
    "chart.tdee": "TDEE",
    "chart.mon": "T2", "chart.tue": "T3", "chart.wed": "T4", "chart.thu": "T5",
    "chart.fri": "T6", "chart.sat": "T7", "chart.sun": "CN",
    "disease.gout": "Gout",
    "disease.diabetes": "Tiểu đường",
    "disease.hypertension": "Huyết áp cao",
    "disease.high_cholesterol": "Mỡ máu cao",
    "disease.fatty_liver": "Gan nhiễm mỡ",
    "disease.stomach": "Bệnh dạ dày",
    "disease.kidney": "Bệnh thận",
    "disease.other": "Khác",
    "sch.opt_eat": "Tôi sẽ ăn món này",
    "sch.opt_change": "Đổi sang món khác",
    "sch.opt_skip": "Tôi sẽ không ăn bữa này",
    "sch.skipped_badge": "Đã bỏ bữa",
    "sch.skip_saved": "Đã đánh dấu bỏ bữa này",
    "sch.skip_undo": "Đã bỏ đánh dấu bữa này",
    "toast.logging_out": "Đang đăng xuất...",
    "toast.diet_load_fail": "Không thể tải dữ liệu lộ trình",
    "toast.menu_updated": "Đã cập nhật thực đơn mới từ AI!",
    "toast.no_change": "Bạn chưa thay đổi món nào",
    "toast.recalc": "Đang tính lại dinh dưỡng món bạn đổi...",
    "toast.reload_plan": "Đang tải lại lộ trình mới...",
    "toast.update_ok": "Đã cập nhật & tính lại dinh dưỡng!",
    "toast.save_net_err": "Lỗi kết nối khi lưu",
    "toast.coach_net_err": "Lỗi kết nối HLV AI",
    "toast.coach_gen_err": "Chưa tạo được thực đơn, bạn thử lại sau nhé!",
    "sch.generating": "HLV AI đang soạn thực đơn tuần — bạn cứ dùng trang bình thường, bảng sẽ tự cập nhật khi xong.",
    "sch.dont_close": "Vui lòng không đóng trang…",
    "gp.done": "Đã xong!",
    "sch.incomplete": "Thực đơn tuần còn thiếu {n} bữa nên bảng có ô để trống.",
    "sch.fill_missing": "Bù bữa còn thiếu",
    "sch.filling": "Đang bù…",
    "sch.cell_pending": "Đang soạn bữa này",
    "sch.cell_empty": "Chưa có món",
    "survey.prompt_title": "Bạn thấy Dr.Fit thế nào?",
    "survey.prompt_desc": "Dành 3 phút chia sẻ trải nghiệm để đội ngũ cải thiện đúng thứ bạn cần.",
    "survey.prompt_cta": "Đánh giá ngay",
    "survey.prompt_later": "Để sau",
    "guide.back_dashboard": "Về Dashboard",
    "guide.login_to_continue": "Đăng nhập để tiếp tục",
    "guide.toc_title": "Mục lục",
    "guide.toc_intro": "Giới thiệu",
    "guide.toc_start": "Bắt đầu nhanh",
    "guide.toc_setup": "Thiết lập hồ sơ",
    "guide.toc_dashboard": "Dashboard Diet",
    "guide.toc_chat": "Trò chuyện AI",
    "guide.toc_plan": "Lịch 7 ngày",
    "guide.toc_tips": "Mẹo & Lưu ý",
    "guide.toc_faq": "Câu hỏi thường gặp",
    "guide.label_tip": "Mẹo:",
    "guide.label_note": "Lưu ý:",
    "guide.intro_title": "Dr.Fit là gì?",
    "guide.intro_desc": "Dr.Fit là trợ lý dinh dưỡng AI hỗ trợ người dùng, đặc biệt là người có bệnh nền, theo dõi và kiểm soát chế độ ăn uống hằng ngày nhằm góp phần hỗ trợ quá trình cải thiện và điều trị bệnh thông qua dinh dưỡng.<br><br>Người dùng chỉ cần nhập thông tin cơ thể và mục tiêu, hệ thống sẽ tự động phân tích, tính toán dinh dưỡng và gợi ý thực đơn phù hợp theo từng tình trạng sức khỏe.",
    "guide.chip_read5": "Đọc trong 5 phút",
    "guide.chip_beginner": "Phù hợp người mới",
    "guide.chip_updated": "Cập nhật liên tục",
    "guide.feat_auto_t": "Tính toán tự động",
    "guide.feat_auto_d": "BMR, TDEE, mục tiêu calo & macro theo công thức chuẩn.",
    "guide.feat_coach_t": "HLV AI 24/7",
    "guide.feat_coach_d": "Hỏi đáp về món ăn, thay thế nguyên liệu, ghi nhận bữa ăn.",
    "guide.feat_plan_t": "Lịch 7 ngày",
    "guide.feat_plan_d": "Thực đơn xoay vòng, có thể yêu cầu AI sinh lại theo sở thích.",
    "guide.feat_progress_t": "Tiến độ trực quan",
    "guide.feat_progress_d": "Biểu đồ cân nặng, calo và macro theo thời gian thực.",
    "guide.start_title": "Bắt đầu trong 4 bước",
    "guide.start_lead": "Quy trình từ lúc đăng ký đến khi nhận thực đơn cá nhân hoá.",
    "guide.start_s1_t": "1. Đăng ký tài khoản",
    "guide.start_s1_d": "Tạo tài khoản để bắt đầu lưu dữ liệu dinh dưỡng cá nhân.",
    "guide.start_s2_t": "2. Khai báo hồ sơ",
    "guide.start_s2_d": "Nhập thông tin cơ thể, mức vận động, mục tiêu (giảm cân / tăng cơ / duy trì) hoặc tình trạng sức khỏe.",
    "guide.start_s3_t": "3. Nhận đề xuất",
    "guide.start_s3_d": "Hệ thống tự động tính BMR, TDEE và xây dựng chế độ ăn phù hợp với thể trạng.",
    "guide.start_s4_t": "4. Theo dõi mỗi ngày",
    "guide.start_s4_d": "Ghi nhận bữa ăn hằng ngày qua chat, theo dõi lượng calo và macro đã nạp, đồng thời kiểm soát tiến độ so với mục tiêu. Thực đơn được tự động điều chỉnh linh hoạt dựa trên dữ liệu ăn uống thực tế của người dùng.",
    "guide.setup_title": "Thiết lập hồ sơ (Setup)",
    "guide.setup_lead": "Tại đây trang chia làm 4 bước — biểu tượng tiến độ ở trên cùng cho biết bạn đang ở đâu.",
    "guide.setup_fields_h": "Các trường thông tin",
    "guide.setup_li1": "<strong>Thông tin cơ bản:</strong> tên, tuổi, giới tính.",
    "guide.setup_li2": "<strong>Chỉ số cơ thể:</strong> chiều cao (cm), cân nặng hiện tại (kg).",
    "guide.setup_li3": "<strong>Mức vận động:</strong> ít vận động → vận động nặng (ảnh hưởng TDEE).",
    "guide.setup_li4": "<strong>Mục tiêu:</strong> Giảm cân · Duy trì · Tăng cơ — chọn 1 trong các thẻ Goal.",
    "guide.setup_tip_body": "Cập nhật cân nặng mỗi tuần để biểu đồ tiến độ phản ánh đúng thực tế.",
    "guide.dash_title": "Dashboard Diet",
    "guide.dash_lead": "Tại đây trang tổng hợp toàn bộ chỉ số quan trọng dưới dạng biểu đồ trực quan.",
    "guide.dash_macro_t": "Macro Donut",
    "guide.dash_macro_d": "Tỉ lệ Protein / Carbs / Fat đề xuất.",
    "guide.dash_weight_t": "Tiến độ cân nặng",
    "guide.dash_weight_d": "Đường biểu diễn cân theo tuần.",
    "guide.dash_cal_t": "Calo theo ngày",
    "guide.dash_cal_d": "Cột calo nạp 7 ngày gần nhất.",
    "guide.dash_bmr_t": "BMR vs TDEE",
    "guide.dash_bmr_d": "So sánh năng lượng cơ bản và tiêu hao.",
    "guide.dash_warn_body": "Nếu các chip BMR/TDEE hiển thị \"--\", bạn cần hoàn tất Setup trước.",
    "guide.chat_title": "Trò chuyện với HLV AI",
    "guide.chat_lead": "Đây là nơi bạn ghi nhận bữa ăn và nhận tư vấn theo ngữ cảnh.",
    "guide.chat_ask_h": "Bạn có thể hỏi gì?",
    "guide.chat_q1": "\"Tôi vừa ăn 1 bát phở bò, ghi nhận giúp tôi.\"",
    "guide.chat_q2": "\"Bữa tối hôm nay nên ăn gì để đủ Protein?\"",
    "guide.chat_q3": "\"Có thể thay thịt bò bằng món chay nào?\"",
    "guide.chat_q4": "\"Hôm nay tôi còn bao nhiêu calo?\"",
    "guide.chat_note_body": "Khi AI nhận diện món ăn, sẽ hiện hộp xác nhận trước khi lưu vào nhật ký để đảm bảo dữ liệu chính xác.",
    "guide.plan_title": "Lịch ăn 7 ngày",
    "guide.plan_lead": "Đây là nơi hiển thị thực đơn dạng bảng theo ngày × bữa.",
    "guide.plan_li1": "Mỗi ô là một bữa được AI gợi ý phù hợp với mục tiêu calo & macro.",
    "guide.plan_li2": "Bấm <strong>\"Sinh lại lịch\"</strong> để AI tạo phương án mới.",
    "guide.plan_li3": "Kéo thanh bên dưới để xem các phản hồi/giải thích từ AI về kế hoạch.",
    "guide.tips_title": "Mẹo dùng hiệu quả",
    "guide.tips_dash_t": "Vào Dashboard",
    "guide.tips_dash_d": "Kiểm tra chỉ số mỗi sáng để có kế hoạch ăn phù hợp trong ngày.",
    "guide.tips_dash_go": "Mở Diet",
    "guide.tips_chat_t": "Ghi nhận bữa ăn",
    "guide.tips_chat_d": "Nhập càng sớm càng tốt sau khi ăn để dữ liệu được chính xác.",
    "guide.tips_chat_go": "Mở Chat",
    "guide.tips_plan_t": "Lên kế hoạch tuần",
    "guide.tips_plan_d": "Sinh lịch vào đầu tuần để chuẩn bị nguyên liệu trước.",
    "guide.tips_plan_go": "Mở Plan",
    "guide.tips_setup_t": "Cập nhật hồ sơ",
    "guide.tips_setup_d": "Thay đổi cân nặng/mục tiêu định kỳ để AI tính lại chỉ số.",
    "guide.tips_setup_go": "Mở Setup",
    "guide.faq_title": "Câu hỏi thường gặp",
    "guide.faq_q1": "Dữ liệu của tôi có được bảo mật?",
    "guide.faq_a1": "Toàn bộ dữ liệu cá nhân được lưu trên tài khoản riêng và chỉ bạn truy cập được sau khi đăng nhập.",
    "guide.faq_q2": "Tại sao calo tôi nhập không khớp với AI tính?",
    "guide.faq_a2": "AI ước lượng theo khẩu phần trung bình. Nếu món của bạn lớn/nhỏ hơn, hãy nói rõ trong chat (ví dụ: \"1 bát phở lớn 500ml\").",
    "guide.faq_q3": "Tôi có thể đổi mục tiêu sau khi setup?",
    "guide.faq_a3": "Có. Vào trang Setup → chọn mục tiêu mới → AI sẽ tính lại calo và macro mục tiêu.",
    "guide.faq_q4": "Lịch 7 ngày có thay đổi mỗi tuần không?",
    "guide.faq_a4": "Bạn có thể bấm \"Sinh lại\" bất kỳ lúc nào để có thực đơn mới phù hợp khẩu vị/mùa.",
    "diary.title": "Nhật ký ảnh món ăn",
    "diary.sub": "Những món bạn đã chụp và phân tích gần đây.",
    "diary.empty": "Chưa có ảnh nào — hãy chụp món ăn ở Chat hoặc trang Kế hoạch.",
    "week.title": "Thống kê 7 ngày gần nhất",
    "week.sub": "Các chất đã nạp mỗi ngày so với mức khuyến nghị & cảnh báo sức khỏe từ AI",
    "week.chart_title": "Chất dinh dưỡng đã nạp vs khuyến nghị",
    "week.no_data": "Chưa có dữ liệu — hãy tick \"Đã ăn\" ở trang Kế hoạch để hệ thống thống kê.",
    "week.health_title": "Cảnh báo sức khỏe 7 ngày",
    "week.health_desc": "AI phân tích hành vi ăn uống 7 ngày gần nhất, dự đoán xu hướng tình trạng bệnh và đưa lời khuyên.",
    "week.analyze": "Phân tích bằng AI",
    "week.analyzing": "AI đang phân tích...",
    "week.advice": "Lời khuyên",
    "week.status_good": "Đang cải thiện tốt",
    "week.status_stable": "Duy trì ổn định",
    "week.status_risk": "Có nguy cơ xấu đi",
    "plan.congrat_title": "Chúc mừng bạn!",
    "plan.congrat_body": "Bạn đã hoàn thành xuất sắc chặng đường dinh dưỡng. Hãy cập nhật lại chỉ số mới để AI thiết kế lộ trình tiếp theo nhé!",
    "plan.congrat_btn": "Tiếp tục chặng đường mới",
    "toast.login_required": "Vui lòng đăng nhập!",
    "toast.estimate_fail": "Không ước tính được",
    "toast.estimate_net_err": "Lỗi kết nối khi ước tính",
  },

  en: {
    "common.lang": "Language",
    "common.save": "Save changes",
    "common.cancel": "Cancel",
    "common.close": "Close",
    "common.add": "Add",
    "common.delete": "Delete",
    "common.loading": "Loading...",
    "common.processing": "Processing...",
    "common.saving": "Saving...",
    "common.creating": "Creating...",
    "common.uploading": "Uploading...",
    "common.sending": "Sending...",
    "common.deleting": "Deleting...",
    "common.logout": "Log out",
    "common.logout_hint": "Click to log out",
    "common.kcal": "kcal",
    "common.today": "Today",
    "nav.ai": "AI",
    "nav.diet": "DIET",
    "nav.plan": "PLAN",
    "nav.family": "FAMILY",
    "nav.profile": "PROFILE",
    "nav.guide": "GUIDE",
    "land.signin": "Sign in",
    "land.signup": "Sign up",
    "land.badge": "Next-generation artificial intelligence",
    "land.title1": "Calorie tracking",
    "land.title2": "Easier than ever before.",
    "land.desc": "Just send a photo or describe your meal — the system analyzes its nutrition, gives tailored suggestions, and adapts your eating plan to your goals and real habits.",
    "land.cta": "Get started",
    "land.footer": "© 2026 Dr.Fit. Smart nutrition analysis.",
    "chat.greeting": "Hi there! Send a message or a photo of your food and I'll analyze it for you.",
    "chat.placeholder": "Ask me anything about nutrition...",
    "chat.total_energy": "Total energy",
    "chat.info_update": "Details will appear after analysis.",
    "chat.protein": "Protein",
    "chat.fat": "Fat",
    "chat.carbs": "Carbs",
    "chat.fiber": "Fiber",
    "chat.sugar": "Sugar",
    "chat.sodium": "Sodium",
    "chat.upload_hint": "Upload a food photo",
    "chat.send": "Send",
    "chat.analyze_image": "Analyze this image",
    "chat.analyzing": "AI is analyzing...",
    "chat.loading_history": "Loading history...",
    "chat.pasted_image": "Image pasted! Press Send to analyze.",
    "chat.pasted_label": "Pasted image (Ctrl+V)",
    "chat.remove_image": "Remove image",
    "chat.jpg_only": "Only JPG images are allowed!",
    "chat.sent_image": "[Image sent]",
    "chat.voice_unsupported": "Your browser doesn't support voice input.",
    "chat.voice_denied": "Please allow microphone access.",
    "chat.voice_nospeech": "I didn't hear anything — try again.",
    "chat.voice_neterr": "Network error during voice recognition.",
    "common.please_login": "Please sign in!",
    "common.logging_out": "Signing out...",
    "common.error_generic": "Something went wrong!",
    "common.session_expired": "Your session has expired. Please sign in again.",
    "meal.confirm_title": "Confirm your meal",
    "meal.choose_time": "Choose meal",
    "meal.breakfast": "Breakfast",
    "meal.lunch": "Lunch",
    "meal.dinner": "Dinner",
    "meal.snack": "Snack",
    "meal.when": "When",
    "meal.today": "Today",
    "meal.other_day": "Another day",
    "meal.confirm": "Confirm",
    "meal.cancel": "Cancel",
    "meal.pick_time": "Please choose a meal",
    "meal.cancelled": "Cancelled. You can enter another dish.",
    "meal.updating": "Updating your menu...",
    "meal.logged": "I've logged your meal!",
    "meal.update_fail": "I couldn't update it — please try again.",
    "meal.updated_toast": "Timetable & stats updated!",
    "meal.conn_err": "Server connection error",
    "meal.confirm_sent": "Confirmed: eaten at {meal}, {day}",
    "sch.hero_title": "Your meal plan",
    "sch.hero_desc": "AI builds a 7-day schedule based on your goals, taste and activity level",
    "sch.coach_title": "AI Nutrition Coach",
    "sch.coach_desc": "Chat with the AI to update your menu and track your meals.",
    "sch.week_title": "7-day meal plan",
    "sch.meal": "Meal",
    "sch.breakfast": "Breakfast",
    "sch.lunch": "Lunch",
    "sch.dinner": "Dinner",
    "sch.snack": "Snack",
    "sch.eaten": "Eaten",
    "sch.eaten_q": "Have you eaten this meal?",
    "sch.mark_eaten": "Mark as eaten",
    "sch.will_eat": "I'll eat this dish",
    "sch.alt_ph": "What would you like to eat instead?",
    "sch.confirm_change": "Confirm swap",
    "sch.alt_hint": "Type the dish you want, then confirm",
    "sch.find_near": "Find nearby places",
    "sch.ask_ai": "Ask the AI Coach",
    "sch.nutrition_struct": "Nutrition breakdown",
    "today.title": "What you've eaten today",
    "today.subtitle": "Total energy & nutrition consumed today",
    "today.consumed": "Consumed",
    "today.target": "Target",
    "today.remaining": "Remaining",
    "today.over": "Over",
    "today.no_meal": "You haven't marked any meal today. Tick \"Eaten\" on each meal to track it.",
    "extra.add_btn": "Add food outside the plan",
    "extra.title": "Add an extra food",
    "extra.desc": "Snacks, fruit, drinks… outside the plan? Add them here to count toward today's total.",
    "extra.name": "Food name",
    "extra.name_ph": "e.g. Apple, yogurt, milk tea...",
    "extra.kcal": "Energy (kcal)",
    "extra.kcal_ph": "e.g. 95",
    "extra.estimate": "Auto-estimate with AI",
    "extra.estimating": "AI is estimating...",
    "extra.low_conf": "Values are rough estimates only.",
    "extra.detected": "Detected",
    "extra.filled_note": "Filled into the form — press <b>Add</b> to save",
    "extra.server_err": "Server error ({status}). Please try again!",
    "extra.added": "Added to today!",
    "extra.list_title": "Today's extra foods",
    "extra.row": "Extras",
    "extra.need_name": "Please enter a food name",
    "extra.optional_macro": "Nutrition (optional)",
    "extra.upload_photo": "Upload a food photo",
    "extra.analyzing_photo": "AI is analyzing the photo...",
    "extra.photo_done": "Photo analyzed! Please double-check the numbers.",
    "extra.photo_fail": "Couldn't analyze the photo",
    "extra.photo_net_err": "Couldn't analyze the photo. Check your connection!",
    "extra.not_food": "That photo doesn't look like food. Try another one.",
    "extra.photo_hint": "or snap/upload a photo for AI to detect",
    "rem.title": "Reminders",
    "rem.subtitle": "Medication & meal-time reminders",
    "rem.tab_meal": "Meals",
    "rem.tab_med": "Medication",
    "rem.add": "Add reminder",
    "rem.label": "Reminder text",
    "rem.label_meal_ph": "e.g. Breakfast",
    "rem.label_med_ph": "e.g. Take vitamin D",
    "rem.time": "Time",
    "rem.repeat": "Repeat daily",
    "rem.none": "No reminders yet.",
    "rem.enable_notif": "Enable browser notifications",
    "rem.notif_blocked": "Notifications are blocked. Please re-enable them in settings.",
    "rem.notif_on": "Notifications enabled!",
    "rem.saved": "Reminder saved",
    "rem.deleted": "Reminder deleted",
    "rem.fire_meal": "🍽️ Time to eat",
    "rem.fire_med": "💊 Time for your medication",
    "rem.need_time": "Please choose a time",
    "rem.open": "Reminders",
    "rem.alarm_now": "NOW",
    "rem.alarm_dismiss": "Got it",
    "rem.alarm_default_med": "It's time to take your medication.",
    "rem.alarm_default_meal": "It's time to eat.",
    "setup.body_title": "Body metrics",
    "setup.body_desc": "To calculate your BMR and TDEE accurately.",
    "setup.gender": "Gender",
    "setup.male": "Male",
    "setup.female": "Female",
    "setup.birth": "Year of birth",
    "setup.height": "Height (cm)",
    "setup.weight": "Weight (kg)",
    "setup.goal_title": "Your goals",
    "setup.goal_desc": "You can select more than one goal.",
    "setup.goal_lose": "Lose weight",
    "setup.goal_maintain": "Maintain weight",
    "setup.goal_gain": "Gain weight",
    "setup.goal_muscle": "Build muscle",
    "setup.goal_disease": "Improve / support a health condition",
    "setup.disease_label": "Disease / health condition",
    "setup.disease_pick": "-- Select a condition --",
    "setup.disease_other_ph": "Enter condition name...",
    "setup.target_weight": "Target weight",
    "setup.deadline": "Deadline",
    "setup.speed": "Desired pace",
    "setup.speed_safe": "Safe (0.5kg/week)",
    "setup.speed_normal": "Moderate (0.7kg/week)",
    "setup.speed_fast": "Fast (1kg/week)",
    "setup.habit_title": "Habits & flexibility",
    "setup.habit_desc": "Optimize calories around your schedule.",
    "setup.activity": "Activity level",
    "setup.act_1": "Sedentary (office)",
    "setup.act_2": "Light (1-2 workouts/week)",
    "setup.act_3": "Moderate (3-5 workouts/week)",
    "setup.act_4": "Heavy (6-7 workouts/week)",
    "setup.cheat": "High-calorie days (cheat days)?",
    "setup.cheat_ph": "e.g. Saturday and Sunday",
    "setup.snack_q": "Do you snack often?",
    "setup.snack_no": "Never",
    "setup.snack_sometimes": "Sometimes",
    "setup.snack_often": "Often",
    "setup.deep_title": "Deep personalization",
    "setup.deep_desc": "The final step to complete your plan.",
    "setup.allergies": "Allergies / foods you avoid",
    "setup.allergies_ph": "e.g. No onion, seafood allergy...",
    "setup.focus_q": "Which macro do you want to focus on?",
    "setup.focus_balanced": "Balanced",
    "setup.focus_protein": "High protein",
    "setup.focus_lowcarb": "Low carb",
    "setup.reason": "Why are you starting?",
    "setup.reason_ph": "Improve my health...",
    "setup.prev": "Back",
    "setup.next": "Continue",
    "setup.finish": "Finish setup",
    "setup.extend_banner": "Your previous journey is complete. Update your new weight and set a new deadline!",
    "setup.pick_goal": "Please select at least one goal.",
    "diet.today_route": "Today's plan",
    "diet.daily_target": "Daily intake target",
    "diet.weight_now": "Current weight",
    "diet.target_to": "Target weight",
    "diet.deadline": "Deadline",
    "diet.nutri_title": "Nutrition analysis",
    "diet.nutri_sub": "Macro ratio & weight progress along your plan",
    "diet.macro_ratio": "Recommended macro ratio",
    "diet.weight_progress": "Weight progress",
    "diet.cal_per_day": "Calories per day of the week",
    "auth.signin_title": "Welcome back",
    "auth.signin_sub": "Keep tracking your health",
    "auth.username": "Username",
    "auth.username_ph_signin": "Enter your username",
    "auth.password": "Password",
    "auth.signin_btn": "Sign in",
    "auth.no_account": "Don't have an account?",
    "auth.signup_now": "Sign up now",
    "auth.signup_title": "Create a new account",
    "auth.signup_sub": "Start your nutrition journey",
    "auth.username_ph_signup": "e.g. johndoe",
    "auth.birth": "Year of birth",
    "auth.weight": "Weight (kg)",
    "auth.height": "Height (cm)",
    "auth.signup_btn": "Sign up",
    "auth.have_account": "Already have an account?",
    "auth.signin_link": "Sign in",
    "diet.personalized": "Personalized",
    "diet.kcal_per_day": "kcal / day",
    "diet.disease_eyebrow": "Conditions to watch",
    "diet.disease_loading": "Loading your health info...",
    "diet.disease_loading_desc": "We'll personalize your menu based on your health conditions.",
    "diet.disease_none_title": "No underlying conditions",
    "diet.disease_none_desc": "You haven't reported any condition. Your menu is optimized for weight & energy goals.",
    "diet.disease_one_title": "Dietary notes for your condition",
    "diet.disease_many_title": "You have {n} conditions to watch",
    "diet.disease_warn_desc": "Please choose suitable foods. The system will flag foods that aren't good for the conditions below.",
    "chart.protein": "Protein",
    "chart.carbs": "Carbs",
    "chart.fats": "Fats",
    "chart.weight_kg": "Weight (kg)",
    "chart.target": "Target",
    "chart.start": "Start",
    "chart.current": "Now",
    "chart.week_n": "Week {n}",
    "chart.cal_intake_est": "Calories (est.)",
    "chart.bmr_basal": "BMR (basal)",
    "chart.activity": "Activity",
    "chart.bmr": "BMR",
    "chart.tdee": "TDEE",
    "chart.mon": "Mon", "chart.tue": "Tue", "chart.wed": "Wed", "chart.thu": "Thu",
    "chart.fri": "Fri", "chart.sat": "Sat", "chart.sun": "Sun",
    "disease.gout": "Gout",
    "disease.diabetes": "Diabetes",
    "disease.hypertension": "High blood pressure",
    "disease.high_cholesterol": "High cholesterol",
    "disease.fatty_liver": "Fatty liver",
    "disease.stomach": "Stomach problems",
    "disease.kidney": "Kidney disease",
    "disease.other": "Other",
    "sch.opt_eat": "I'll eat this dish",
    "sch.opt_change": "Change to another dish",
    "sch.opt_skip": "I'll skip this meal",
    "sch.skipped_badge": "Skipped",
    "sch.skip_saved": "Marked this meal as skipped",
    "sch.skip_undo": "Unmarked this meal",
    "toast.logging_out": "Logging out...",
    "toast.diet_load_fail": "Couldn't load your plan data",
    "toast.menu_updated": "Menu updated from AI!",
    "toast.no_change": "You haven't changed any dish",
    "toast.recalc": "Recalculating nutrition for your new dish...",
    "toast.reload_plan": "Reloading your new plan...",
    "toast.update_ok": "Updated & recalculated nutrition!",
    "toast.save_net_err": "Network error while saving",
    "toast.coach_net_err": "AI Coach connection error",
    "toast.coach_gen_err": "Couldn't build your menu — please try again shortly!",
    "sch.generating": "AI Coach is building your weekly menu — keep using the page, the table updates itself when it's done.",
    "sch.dont_close": "Please keep this page open…",
    "gp.done": "Done!",
    "sch.incomplete": "Your weekly menu is still missing {n} meals, so some cells are empty.",
    "sch.fill_missing": "Fill missing meals",
    "sch.filling": "Filling…",
    "sch.cell_pending": "Building this meal",
    "sch.cell_empty": "No dish yet",
    "survey.prompt_title": "How is Dr.Fit working for you?",
    "survey.prompt_desc": "Spend 3 minutes sharing your experience so the team improves what actually matters to you.",
    "survey.prompt_cta": "Rate now",
    "survey.prompt_later": "Later",
    "guide.back_dashboard": "Back to Dashboard",
    "guide.login_to_continue": "Sign in to continue",
    "guide.toc_title": "Contents",
    "guide.toc_intro": "Introduction",
    "guide.toc_start": "Quick start",
    "guide.toc_setup": "Set up profile",
    "guide.toc_dashboard": "Diet Dashboard",
    "guide.toc_chat": "AI Chat",
    "guide.toc_plan": "7-day plan",
    "guide.toc_tips": "Tips & Notes",
    "guide.toc_faq": "FAQ",
    "guide.label_tip": "Tip:",
    "guide.label_note": "Note:",
    "guide.intro_title": "What is Dr.Fit?",
    "guide.intro_desc": "Dr.Fit is an AI nutrition assistant that helps users — especially those with underlying conditions — track and manage their daily diet to support recovery and treatment through nutrition.<br><br>Just enter your body metrics and goals, and the system automatically analyzes, calculates nutrition and suggests menus tailored to each health condition.",
    "guide.chip_read5": "5-minute read",
    "guide.chip_beginner": "Beginner friendly",
    "guide.chip_updated": "Continuously updated",
    "guide.feat_auto_t": "Automatic calculation",
    "guide.feat_auto_d": "BMR, TDEE, calorie & macro targets using standard formulas.",
    "guide.feat_coach_t": "AI Coach 24/7",
    "guide.feat_coach_d": "Ask about dishes, swap ingredients, and log meals.",
    "guide.feat_plan_t": "7-day plan",
    "guide.feat_plan_d": "A rotating menu; ask AI to regenerate it to your taste.",
    "guide.feat_progress_t": "Visual progress",
    "guide.feat_progress_d": "Real-time charts for weight, calories and macros.",
    "guide.start_title": "Get started in 4 steps",
    "guide.start_lead": "The journey from sign-up to receiving your personalized menu.",
    "guide.start_s1_t": "1. Create an account",
    "guide.start_s1_d": "Create an account to start saving your personal nutrition data.",
    "guide.start_s2_t": "2. Fill in your profile",
    "guide.start_s2_d": "Enter your body metrics, activity level, goals (lose weight / build muscle / maintain) or health conditions.",
    "guide.start_s3_t": "3. Get recommendations",
    "guide.start_s3_d": "The system automatically computes BMR, TDEE and builds a diet suited to your body.",
    "guide.start_s4_t": "4. Track every day",
    "guide.start_s4_d": "Log daily meals via chat, track the calories and macros you've consumed, and monitor progress toward your goal. The menu adapts flexibly based on your actual eating data.",
    "guide.setup_title": "Set up your profile (Setup)",
    "guide.setup_lead": "This setup guide has 4 steps — the progress icons at the top show where you are.",
    "guide.setup_fields_h": "Information fields",
    "guide.setup_li1": "<strong>Basic info:</strong> name, age, gender.",
    "guide.setup_li2": "<strong>Body metrics:</strong> height (cm), current weight (kg).",
    "guide.setup_li3": "<strong>Activity level:</strong> sedentary → very active (affects TDEE).",
    "guide.setup_li4": "<strong>Goal:</strong> Lose weight · Maintain · Build muscle — pick one of the Goal cards.",
    "guide.setup_tip_body": "Update your weight weekly so the progress chart reflects reality.",
    "guide.dash_title": "Diet Dashboard",
    "guide.dash_lead": "This dashboard brings together all key metrics through visual charts.",
    "guide.dash_macro_t": "Macro Donut",
    "guide.dash_macro_d": "Recommended Protein / Carbs / Fat ratio.",
    "guide.dash_weight_t": "Weight progress",
    "guide.dash_weight_d": "Weight trend line by week.",
    "guide.dash_cal_t": "Calories by day",
    "guide.dash_cal_d": "Calorie bars for the last 7 days.",
    "guide.dash_bmr_t": "BMR vs TDEE",
    "guide.dash_bmr_d": "Compare basal and total energy expenditure.",
    "guide.dash_warn_body": "If the BMR/TDEE chips show \"--\", you need to complete Setup first.",
    "guide.chat_title": "Chat with the AI Coach",
    "guide.chat_lead": "This is where you log meals and get contextual advice.",
    "guide.chat_ask_h": "What can you ask?",
    "guide.chat_q1": "\"I just ate a bowl of beef pho, please log it.\"",
    "guide.chat_q2": "\"What should I eat for dinner to get enough protein?\"",
    "guide.chat_q3": "\"What vegetarian dish can replace beef?\"",
    "guide.chat_q4": "\"How many calories do I have left today?\"",
    "guide.chat_note_body": "When AI recognizes a dish, a confirmation box appears before saving to your log to keep the data accurate.",
    "guide.plan_title": "7-day meal plan",
    "guide.plan_lead": "This is where the meal plan is displayed as a table of day × meal.",
    "guide.plan_li1": "Each cell is a meal suggested by AI to fit your calorie & macro targets.",
    "guide.plan_li2": "Tap <strong>\"Regenerate plan\"</strong> for AI to create a new option.",
    "guide.plan_li3": "Drag the bar below to see AI feedback/explanations about the plan.",
    "guide.tips_title": "Tips for effective use",
    "guide.tips_dash_t": "Open Dashboard",
    "guide.tips_dash_d": "Check your metrics each morning to plan suitable meals for the day.",
    "guide.tips_dash_go": "Open Diet",
    "guide.tips_chat_t": "Log meals",
    "guide.tips_chat_d": "Enter meals as soon as possible after eating for accurate data.",
    "guide.tips_chat_go": "Open Chat",
    "guide.tips_plan_t": "Plan your week",
    "guide.tips_plan_d": "Generate the plan at the start of the week to prep ingredients ahead.",
    "guide.tips_plan_go": "Open Plan",
    "guide.tips_setup_t": "Update profile",
    "guide.tips_setup_d": "Change your weight/goals periodically so AI recalculates your metrics.",
    "guide.tips_setup_go": "Open Setup",
    "guide.faq_title": "Frequently asked questions",
    "guide.faq_q1": "Is my data secure?",
    "guide.faq_a1": "All personal data is stored on your own account and only you can access it after signing in.",
    "guide.faq_q2": "Why don't my entered calories match the AI's calculation?",
    "guide.faq_a2": "AI estimates based on average portions. If your dish is larger/smaller, say so in chat (e.g. \"a large 500ml bowl of pho\").",
    "guide.faq_q3": "Can I change my goal after setup?",
    "guide.faq_a3": "Yes. Go to Setup → choose a new goal → AI will recalculate your target calories and macros.",
    "guide.faq_q4": "Does the 7-day plan change each week?",
    "guide.faq_a4": "You can tap \"Regenerate\" anytime to get a new menu suited to your taste/season.",
    "diary.title": "Food photo diary",
    "diary.sub": "Dishes you've recently photographed and analyzed.",
    "diary.empty": "No photos yet — snap a dish in Chat or the Plan page.",
    "week.title": "Last 7 days overview",
    "week.sub": "Daily nutrient intake vs recommendations & AI health warning",
    "week.chart_title": "Nutrients eaten vs recommended",
    "week.no_data": "No data yet — tick \"Eaten\" on the Plan page so the system can track your intake.",
    "week.health_title": "7-day health warning",
    "week.health_desc": "AI analyzes your eating behavior over the last 7 days, predicts your health trend and gives advice.",
    "week.analyze": "Analyze with AI",
    "week.analyzing": "AI is analyzing...",
    "week.advice": "Advice",
    "week.status_good": "Improving well",
    "week.status_stable": "Staying stable",
    "week.status_risk": "At risk of getting worse",
    "plan.congrat_title": "Congratulations!",
    "plan.congrat_body": "You've successfully completed your nutrition journey. Update your new stats so AI can design your next plan!",
    "plan.congrat_btn": "Start a new journey",
    "toast.login_required": "Please sign in!",
    "toast.estimate_fail": "Couldn't estimate",
    "toast.estimate_net_err": "Network error while estimating",

    "fm.need_household": "You need to create a family profile first.",
    "fm.create_now": "Create now →",

    "ml.in_use": "In use",
    "ml.days": "days",
    "ml.back": "All menus",
    "ml.today": "Today's menu",
    "ml.day_empty": "No dishes",
    "ml.detail_empty": "This menu has no days yet.",
    "ml.open_plan": "Open current plan",
    "ml.current_menu": "the current menu",
    "ml.confirm_switch": "Are you sure you want to switch menus?\n\nYou're currently using \"{from}\". Switching to \"{to}\" will replace your current meal plan.\n\nContinue?",
    "ml.title": "Standard Menu Library",
    "ml.subtitle": "AI picks the menu that best fits your family from the existing library — it never invents dishes",
    "ml.all": "All",
    "ml.score": "Match",
    "ml.generate": "Create menu for family",
    "ml.empty": "No matching menu in the library yet — upload a new one below.",
    "ml.add_title": "Add your own menu to the library",
    "ml.add_sub": "Upload an Excel file or enter manually — public to everyone by default",
    "ml.f_name": "Menu name",
    "ml.f_name_ph": "7-day weight-loss menu",
    "ml.f_tags": "Tags (comma-separated)",
    "ml.f_disease": "Target condition",
    "ml.f_disease_ph": "diabetes, gout...",
    "ml.f_scope": "Visibility",
    "ml.scope_public": "Public (default)",
    "ml.scope_private": "My family only",
    "ml.f_excel": "Excel file (.xlsx)",
    "ml.upload_btn": "Upload",
    "ml.download_template": "Download template (.xlsx)",
    "ml.template_downloaded": "Template downloaded — fill it in, then upload!",
    "ml.excel_cols": "Excel columns",
    "ml.toast_generated": "Menu created for your family!",
    "ml.toast_pick_excel": "Pick an Excel file first.",
    "ml.toast_added": "Menu added to the library!",

    "mp.meal_breakfast": "Breakfast",
    "mp.meal_lunch": "Lunch",
    "mp.meal_dinner": "Dinner",
    "mp.meal_snack": "Snack",
    "mp.confirm_regen": "Regenerate the entire menu for this week?",
    "mp.toast_regen": "Weekly menu regenerated!",
    "mp.toast_swapped": "Dish swapped!",
    "mp.title": "Your family's weekly menu",
    "mp.subtitle": "Chosen from the standard library, auto-adjusted for each person's allergies/conditions",
    "mp.no_plan": "Your family has no menu yet.",
    "mp.pick_from_lib": "Pick a menu from the library →",
    "mp.title": "Weekly menu",
    "mp.subtitle": "Tap a day to see its meals, nutrition and shopping list",
    "mp.protein": "Protein",
    "mp.fat": "Fat",
    "mp.carbs": "Carbs",
    "mp.fiber": "Fiber",
    "mp.sugar": "Sugar",
    "mp.sodium": "Sodium",
    "mp.more_dishes": "more dishes",
    "mp.day_empty": "No dishes yet",
    "mp.no_nutrition": "No nutrition data yet",
    "mp.swap": "Swap dish",
    "mp.loading_plan": "Loading menu…",
    "mp.shopping_title": "Ingredients to buy (whole week)",
    "mp.items": "ingredients",
    "mp.est_cost": "estimated",
    "mp.need_estimate": "estimate needed",
    "mp.need_estimate_n": "items need your own estimate",
    "mp.no_price": "items without a price",
    "mp.ingredient": "Ingredient",
    "mp.qty": "Quantity",
    "mp.unit_price": "Unit price",
    "mp.line_total": "Subtotal",
    "mp.merged_from": "Merged from",
    "mp.copy": "Copy",
    "mp.copied": "Copied",
    "mp.expand": "Show all",
    "mp.collapse": "Show less",
    "mp.price_in_file": "Price in the source file",
    "ml.tab_browse": "Library",
    "ml.tab_add": "Add a menu",
    "ml.search": "Search menus",
    "ml.search_ph": "Search by menu name, condition or publisher…",
    "ml.origin_all": "All sources",
    "ml.origin_system": "Built-in",
    "ml.origin_mine": "User created",
    "ml.no_match": "No menu matches your filters. Try clearing the search or picking \"All sources\".",
    "ml.price_note_short": "Estimated for one person — varies with where you shop.",
    "ml.scope_private_org": "My organisation only",
    "mp.price_note": "Prices are an estimate for one person and vary with where you buy the ingredients (supermarket, wet market, convenience store). Treat them as a reference.",
    "mp.opening_maps": "Opening maps…",
    "hh.org_manage": "Your organisation",
    "hh.delete_org_btn": "Delete my organisation",
    "hh.leave_desc_chef": "You manage nutrition profiles for this organisation. Deleting it removes every member profile you created — this cannot be undone.",
    "mp.where_title": "Where to buy near you?",
    "mp.where_desc": "Opens Google Maps around your current location.",
    "mp.shop_supermarket": "Supermarket",
    "mp.shop_market": "Wet market",
    "mp.shop_mall": "Shopping mall",
    "mp.sample_note": "This is a sample menu from",
    "mp.sample_note2": "— reference only, not your family's menu yet.",
    "mp.pick_real": "Pick a menu for your family",
    "mp.need_household": "You don't have a family profile yet — this is a sample menu.",
    "mp.create_household": "Create a family",
    "mp.tab_menu": "Menu",
    "mp.tab_shopping": "Shopping list",
    "mp.regen_week": "Regenerate week",
    "mp.day": "Day",
    "mp.shopping_title": "Ingredients to buy (whole week)",
    "mp.no_ingredients": "No ingredients yet.",
    "mp.ingredient": "Ingredient",
    "mp.quantity": "Quantity",
    "mp.protein": "Protein",
    "mp.fat": "Fat",
    "mp.carbs": "Carbs",
    "mp.swap_dish": "Swap dish",

    "hh.goal_maintain": "Maintain",
    "hh.goal_lose": "Lose weight",
    "hh.goal_gain": "Gain weight",
    "hh.goal_muscle": "Build muscle",
    "hh.toast_created": "Family profile created!",
    "hh.toast_mode_switched": "Mode switched!",
    "hh.toast_need_code": "Enter all 6 digits.",
    "hh.toast_code_copied": "Code copied!",
    "hh.toast_copy_failed": "Couldn't copy — select the code and copy it manually.",
    "hh.toast_code_regenerated": "New code generated!",
    "hh.toast_request_sent": "Request sent — waiting for the family owner's approval.",
    "hh.toast_request_accepted": "New member approved!",
    "hh.toast_request_rejected": "Request rejected.",
    "hh.toast_need_name": "Enter the member's name.",
    "hh.toast_member_saved": "Member saved!",
    "hh.confirm_remove": "Remove this member?",
    "hh.toast_member_removed": "Member removed.",
    "hh.title": "Your family",
    "hh.subtitle": "Manage members and pick the right mode — the menu library auto-adjusts per person",
    "hh.start": "Get started",
    "hh.start_desc": "You don't have a family profile yet. Pick a mode to start:",
    "hh.mode_chef": "Chef mode",
    "hh.mode_chef_desc": "You manage profiles for several people (no login needed for them) — ideal when one person cooks for the whole family.",
    "hh.mode_family": "Family mode",
    "hh.mode_family_desc": "Share a 6-digit code so other members can join with their own accounts.",
    "hh.mode_family_note": "Members can sign in with their own accounts.",
    "hh.mode_chef_note": "You manage nutrition profiles for members (no login needed for them).",
    "hh.switch_mode": "Switch mode",
    "hh.code_title": "Family join code",
    "hh.code_desc": "Share this 6-digit code. Whoever enters it shows up under \"Join requests\" below for you to approve.",
    "hh.copy": "Copy",
    "hh.regenerate": "New code",
    "hh.confirm_regenerate": "Generate a new code? The old one stops working immediately.",
    "hh.requests_title": "Join requests",
    "hh.requests_empty": "No requests yet.",
    "hh.unknown_user": "User",
    "hh.role_member": "Member",
    "hh.membership": "Your membership",
    "hh.family_manage": "Your family",
    "hh.leave_desc_member": "You're a member of this family, so you can't create your own family or generate a join code. Leave the family if you'd like to start your own.",
    "hh.leave_desc_owner": "You're the family owner. To join someone else's family you must delete this one first — every member is removed automatically.",
    "hh.leave_btn": "Leave family",
    "hh.delete_family_btn": "Delete my family",
    "hh.confirm_leave": "Leave this family? You'll lose access to the family's shared menu.",
    "hh.confirm_delete_family": "Delete your family? All of the family's menus will be deleted with it. This cannot be undone.",
    "hh.confirm_delete_family_n": "Delete your family? {n} other member(s) will be removed from it, along with every menu and pending join request. This cannot be undone.",
    "hh.notice_someone": "the family owner",
    "hh.notice_removed": "You've been removed from {who}'s family.",
    "hh.notice_approved": "You've been approved into {who}'s family.",
    "hh.toast_left": "You left the family.",
    "hh.toast_family_deleted": "Family deleted.",
    "hh.toast_family_deleted_n": "Family deleted, {n} member(s) removed.",
    "hh.optional": "optional",
    "hh.f_target_weight_ph": "Not needed for this goal",
    "hh.err_target_required_lose": "The \"Lose weight\" goal needs a target weight.",
    "hh.err_target_required_gain": "The \"Gain weight\" goal needs a target weight.",
    "hh.err_target_number": "Target weight must be a number.",
    "hh.err_target_range": "Target weight must be between {min} and {max} kg.",
    "hh.err_target_lose": "\"Lose weight\": the target must be LOWER than the current weight ({current} kg).",
    "hh.err_target_gain": "\"Gain weight\": the target must be HIGHER than the current weight ({current} kg).",
    "hh.err_target_far": "A target of {target} kg is too far from the current {current} kg. Please double-check.",
    "hh.err_birth_year": "Invalid birth year.",
    "hh.err_height": "Invalid height (must be between 40 and 250 cm).",
    "hh.err_weight": "Invalid weight (must be between 2 and 300 kg).",
    "hh.accept": "Accept",
    "hh.reject": "Reject",
    "hh.just_now": "Just now",
    "hh.mins_ago": "{n} min ago",
    "hh.hours_ago": "{n} h ago",
    "hh.join_title": "Join another family",
    "hh.join_title_new": "Got a join code?",
    "hh.join_desc": "Enter the 6-digit code the family owner shared with you (if they chose Family mode).",
    "hh.join_input_label": "6-digit join code",
    "hh.join_btn": "Join",
    "hh.waiting_title": "Waiting for the family owner's approval",
    "hh.waiting_desc": "Your request has been sent. You'll be added to the family as soon as the owner accepts it.",
    "hh.members": "Members",
    "hh.add_member": "Add member",
    "hh.tag_linked": "Linked",
    "hh.tag_dependent": "Dependent",
    "hh.owner": "Head",
    "hh.edit": "Edit",
    "hh.lib_title": "Menu library",
    "hh.lib_link": "Browse library & create a menu for your family →",
    "hh.lib_link_org": "Browse library & create a menu for your organisation →",
    "hh.confirm_delete_org": "Delete your organisation? {n} member profiles will be deleted along with every menu. This cannot be undone.",
    "hh.toast_org_deleted": "Organisation deleted.",
    "hh.toast_created_org": "Organisation profile created!",
    "hh.edit_member": "Edit member",
    "hh.add_member_title": "Add member",
    "hh.f_display_name": "Display name",
    "hh.f_relation": "Relationship",
    "hh.f_relation_ph": "child, dad, mom, grandpa...",
    "hh.f_birth_year": "Birth year",
    "hh.f_gender": "Gender",
    "hh.male": "Male",
    "hh.female": "Female",
    "hh.f_height": "Height (cm)",
    "hh.f_weight": "Weight (kg)",
    "hh.f_target_weight": "Target weight (kg)",
    "hh.f_goal": "Weight goal",
    "hh.f_activity": "Activity level",
    "hh.act_sedentary": "Sedentary",
    "hh.act_light": "Lightly active",
    "hh.act_moderate": "Moderately active",
    "hh.act_active": "Very active",
    "hh.f_disease": "Condition",
    "hh.f_disease_ph": "diabetes, gout, hypertension...",
    "hh.f_allergies": "Allergies (comma-separated)",
    "hh.f_allergies_ph": "shrimp, peanuts...",
    "hh.f_dislikes": "Disliked foods",
    "hh.f_likes": "Liked foods",
    "hh.save_member": "Save",

    "adm.err_list": "Couldn't load the list",
    "adm.err_conn": "Connection error",
    "adm.denied_msg": "Account {email} doesn't have admin access. Contact an administrator to request it.",
    "adm.checking_access": "Checking access…",
    "adm.denied_title": "Admin area",
    "adm.back_to_guide": "Back to the guide",
    "adm.survey_report": "Survey report",
    "adm.title": "Knowledge Base for AI",
    "adm.desc_html": "Upload PDF documents for the AI to reference during consultations. Pipeline: store the original file in <strong>Supabase Storage</strong> → extract text → chunk → save to Supabase → PostgreSQL builds a <strong>Full Text Search</strong> index (tsvector + GIN + ts_rank). No embeddings. Takes effect immediately, no redeploy needed.",
    "adm.store_ready": "Knowledge store ready",
    "adm.store_notready": "Supabase table not created",
    "adm.unit_docs": "documents",
    "adm.unit_chunks": "chunks",
    "adm.storage": "File storage",
    "adm.notconfigured": "Not configured",
    "adm.cloudinary": "Cloudinary (optional)",
    "adm.on": "On",
    "adm.off": "Off",
    "adm.upload_title": "Upload PDF documents",
    "adm.dropzone": "Click to choose one or more PDF files",
    "adm.selected_count": "{n} files selected",
    "adm.upload_btn": "Upload & process",
    "adm.hint": "Note: scanned-image PDFs (no text) can't be extracted.",
    "adm.docs_title": "Uploaded documents",
    "adm.refresh": "Refresh",
    "adm.th_file": "File",
    "adm.th_size": "Size",
    "adm.th_status": "Status",
    "adm.th_chunks": "Chunks",
    "adm.th_index": "Index",
    "adm.th_download": "File",
    "adm.empty": "No documents yet. Upload a PDF.",
    "adm.dl_cloudinary": "Download PDF (Cloudinary)",
    "adm.dl": "Download PDF",
    "adm.no_file": "No original file to download",
    "adm.st_ready": "Ready",
    "adm.st_error": "Error",
    "adm.st_uploaded": "Received",
    "adm.st_extracting": "Extracting text…",
    "adm.st_chunking": "Chunking…",
    "adm.st_saving": "Saving…",
    "adm.toast_pick": "Please choose one or more PDF files first.",
    "adm.toast_notpdf": "File \"{name}\" is not a PDF.",
    "adm.progress": "Processing file {i}/{n}: \"{name}\"…",
    "adm.err_upload": "Upload failed",
    "adm.toast_uploaded": "✓ \"{name}\": {chunks} chunks (indexed for Full Text Search).",
    "adm.toast_done": "Done: {s} succeeded, {f} failed.",
    "adm.confirm_delete": "Delete this document? Its text chunks and the file on Cloudinary will be removed.",
    "adm.err_delete": "Delete failed",
    "adm.toast_deleted": "Document deleted.",

    // /menu-plan — weekly meal plan + shopping list
    "mp.title": "Weekly meal plan",
    "mp.subtitle": "Tap any day to see its meals, nutrition and shopping list",
    "mp.tab_menu": "Meal plan",
    "mp.tab_shopping": "Shopping list",
    "mp.servings": "Servings",
    "mp.servings_auto": "Auto",
    "mp.export_all": "Export to Excel",
    "mp.export_shopping": "Shopping list only",
    "mp.regen_week": "Regenerate the week",
    "mp.confirm_regen": "Regenerate the whole meal plan for this week?",
    "mp.toast_regen": "Weekly meal plan regenerated!",
    "mp.toast_swapped": "Dish swapped!",
    "mp.toast_exported": "Downloaded",
    "mp.swap": "Swap dish",
    "mp.loading_plan": "Loading meal plan…",
    "mp.day_empty": "No dishes yet",
    "mp.more_dishes": "more dishes",
    "mp.protein": "Protein",
    "mp.fat": "Fat",
    "mp.carbs": "Carbs",
    "mp.fiber": "Fibre",
    "mp.sugar": "Sugar",
    "mp.sodium": "Sodium",
    "mp.no_nutrition": "No nutrition data yet",
    "mp.per_day": "/ day",
    "mp.shopping_title": "Ingredients to buy (whole week)",
    "mp.ingredient": "Ingredient",
    "mp.qty": "Quantity",
    "mp.unit_price": "Unit price",
    "mp.line_total": "Amount",
    "mp.items": "ingredients",
    "mp.est_cost": "estimated",
    "mp.merged_from": "Merged from",
    "mp.need_estimate": "estimate needed",
    "mp.need_estimate_n": "items you need to estimate yourself",
    "mp.no_price": "items without a price",
    "mp.no_ingredients": "No ingredients yet. The current meal plan has no ingredients declared for its dishes.",
    "mp.copy": "Copy",
    "mp.copied": "Copied",
    "mp.where_title": "Where to buy near you?",
    "mp.where_desc": "Opens Google Maps around your current location.",
    "mp.need_household": "You do not have a family profile yet — this is a sample meal plan for reference.",
    "mp.create_household": "Create a family",
    "mp.sample_note": "This is a sample meal plan from",
    "mp.sample_note2": "— reference data, not your family's own plan yet.",
    "mp.pick_real": "Pick a meal plan for your family",

    // /menu-library — menu library
    "ml.title": "Standard menu library",
    "ml.subtitle": "AI picks the menu from the library that best fits your family — it never invents dishes",
    "ml.back": "All menus",
    "ml.all": "All",
    "ml.days": "days",
    "ml.day_empty": "No dishes yet",
    "ml.today": "Today's menu",
    "ml.current_menu": "current menu",
    "ml.in_use": "In use",
    "ml.score": "Match score",
    "ml.generate": "Use this menu",
    "ml.open_plan": "View the plan in use",
    "ml.detail_empty": "This menu has no day data yet.",
    "ml.empty": "No menus in the library yet — upload a new one below.",
    "ml.add_title": "Add a menu to the library yourself",
    "ml.add_sub": "Upload an Excel file or enter it by hand — public to every user by default",
    "ml.excel_free": "You can upload any menu file (a Day × Meal table) — the system detects the structure automatically. Or use the 16-column template to enter full nutrition and ingredients.",
    "ml.download_template": "Download the template (.xlsx)",
    "ml.template_downloaded": "Template downloaded — fill it in and upload it back!",
    "ml.f_name": "Menu name",
    "ml.f_name_ph": "7-day weight-loss menu",
    "ml.f_disease": "Target condition",
    "ml.f_disease_ph": "diabetes, gout…",
    "ml.f_excel": "Excel file (.xlsx)",
    "ml.f_scope": "Scope",
    "ml.f_tags": "Tags (comma separated)",
    "ml.scope_private": "My family only",
    "ml.scope_public": "Public (default)",
    "ml.upload_btn": "Upload",
    "ml.import_ok": "Detection result",
    "ml.ir_sheet": "Sheet",
    "ml.ir_layout": "Layout",
    "ml.ir_scope": "Imported",
    "ml.ir_days": "days",
    "ml.ir_dishes": "dishes",
    "ml.ir_confidence": "Confidence",
    "ml.ir_by_ai": "detected by AI",
    "ml.toast_added": "Menu added to the library!",
    "ml.toast_generated": "Meal plan created for your family!",
    "ml.toast_pick_excel": "Choose an Excel file first.",

    // thư viện — sửa / xoá / phân quyền
    "ml.edit": "Edit",
    "ml.edit_title": "Edit menu",
    "ml.delete": "Delete menu",
    "ml.system": "System",
    "ml.system_hint": "A system menu provided by the app",
    "ml.f_desc": "Short description",
    "ml.f_desc_ph": "7-day low-carb menu, suits type-2 diabetes",
    "ml.f_category": "Category",
    "ml.f_system": "System menu",
    "ml.f_system_hint": "Regular users can only view and apply it, not edit it.",
    "ml.change_image": "Choose a cover image",
    "ml.remove_image": "Remove image",
    "ml.image_hint": "JPEG/PNG/WebP, up to {n} MB. Leave empty to use the category cover.",
    "ml.e_img_size": "The image must be at most {n} MB.",
    "ml.e_title": "The title cannot be empty.",
    "ml.toast_saved": "Menu saved!",
    "ml.toast_deleted": "Menu deleted.",
    "ml.confirm_delete": "Delete the menu \"{title}\"?\n\nIt will disappear from the library. Meal plans already created from it are kept.",

    // thư viện — menu in use, empty state, manual entry, shopping checklist
    "ml.active_now": "Your family is using",
    "ml.active_open": "View menu",
    "ml.start_title": "Start building a menu that fits",
    "ml.start_sub": "Your family does not have a menu yet. Pick one from the library below, or add your own in either of two ways:",
    "ml.start_excel": "Upload an Excel file",
    "ml.start_manual": "Enter a menu by hand",
    "ml.add_or": "or upload an Excel file below",
    "ml.dishes": "dishes",
    "ml.kcal_day": "kcal/day",
    "ml.day_no_nutrition": "This source menu lists dishes only, without nutrition figures. You can still see each dish’s ingredients below.",
    "mp.shop_loading": "Building your shopping list…",
    "ml.needs_review": "The source notes that this meal’s nutrition figures are incomplete — treat them as indicative.",
    "ml.ing": "Ingredient",
    "ml.ing_use": "Amount",
    "ml.an_title": "Ingredient breakdown",
    "ml.an_items": "ingredients",
    "ml.an_note": "Shares are by ingredient weight, not by calories.",
    "ml.an_g_starch": "Starches",
    "ml.an_g_protein": "Protein",
    "ml.an_g_veg": "Vegetables",
    "ml.an_g_fruit": "Fruit",
    "ml.an_g_dairy": "Dairy",
    "ml.an_g_fat": "Fats",
    "ml.an_g_liquid": "Broth & liquids",
    "ml.an_g_season": "Seasoning",
    "ml.an_g_other": "Other",
    "ml.an_h_fiber": "High in fibre",
    "ml.an_h_goodfat": "Healthy fats",
    "ml.an_h_wholegrain": "Whole grains",
    "ml.an_h_plantprotein": "Plant protein",
    "ml.an_h_sodium": "High in sodium",
    "ml.an_h_satfat": "Saturated fat",
    "ml.an_h_redmeat": "Red meat",
    "ml.an_h_addedsugar": "Added sugar",
    "ml.provenance": "Data sources",
    "ml.prov_article": "Original article",
    "ml.prov_published": "Published",
    "ml.prov_reviewer": "Medically reviewed by",
    "ml.prov_audience": "Intended for",
    "ml.prov_target": "Reference portion",
    "ml.prov_nutrition": "Nutrition",
    "ml.prov_price": "Reference prices",
    "ml.prov_date": "Processed on",
    "ml.prov_file": "Source file",
    "ml.see_detail": "View nutrition details",
    "ml.day_modal_note": "Nutrition figures are an estimate for one serving, taken from the publisher’s document. Not a substitute for medical advice.",
    "ml.week_cost": "/ week",
    "ml.shop_title": "Ingredients to buy",
    "ml.shop_sub": "A preview for this menu — you do not have to apply it first.",
    "ml.shop_sub_inuse": "The list for the menu you are currently using.",
    "ml.manual_title": "Enter a menu by hand",
    "ml.manual_hint": "One dish per line. To declare amounts, add them after a colon — for example \"Beef pho: 180 g rice noodles, 50 g beef\" — and the system will turn them into a shopping list.",
    "ml.manual_ph": "One dish per line…",
    "ml.manual_save": "Create menu",
    "ml.manual_empty": "No dishes entered yet",
    "ml.e_no_dish": "Please enter at least one dish.",

    // checklist đi chợ
    "mp.bought": "Bought",
    "mp.notes_title": "Daily shopping notes",
    "mp.notes_sub": "Each note lists what that single day needs — handy for shopping day by day.",
    "mp.need_estimate_hint": "The source menu did not declare an amount for this dish",

    // /settings — account settings + permanent account deletion
    "nav.settings": "SETTINGS",
    "set.title": "Settings",
    "set.subtitle": "Manage your account and personal data",
    "set.account": "Account",
    "set.profile_desc": "Update your body metrics, goals and eating habits.",
    "set.edit_profile": "Edit profile",
    "set.danger": "Danger zone",
    "set.delete_title": "Delete account",
    "set.delete_desc": "Permanently delete your account and all of your data. This action cannot be undone.",
    "set.delete_btn": "Delete account",
    "set.delete_more": "Learn more about account and data deletion on the",
    "set.delete_link": "account deletion page",
    "set.delete_lead": "Deleting your account is permanent. The following will be deleted:",
    "set.delete_irreversible": "This action cannot be undone.",
    "set.d_profile": "Profile",
    "set.d_nutrition": "Nutrition information",
    "set.d_meals": "Meal history and menus",
    "set.d_photos": "Uploaded food images",
    "set.d_chat": "AI conversation history",
    "set.d_family": "Family links",
    "set.d_prefs": "Personal settings",
    "set.type_to_confirm": "Type",
    "set.type_to_confirm2": "to confirm",
    "set.deleting": "Deleting…",
    "set.done_title": "Account deleted",
    "set.done_body": "Your account and all of your data have been permanently deleted. Thank you for using Dr.Fit.",
    "set.done_btn": "Back to sign in",
  },
};

export const DISEASE_KEY_BY_VALUE = {
  "gout": "disease.gout",
  "tiểu đường": "disease.diabetes", "tieu duong": "disease.diabetes", "diabetes": "disease.diabetes",
  "huyết áp cao": "disease.hypertension", "cao huyết áp": "disease.hypertension",
  "huyet ap cao": "disease.hypertension", "high blood pressure": "disease.hypertension", "hypertension": "disease.hypertension",
  "mỡ máu cao": "disease.high_cholesterol", "mo mau cao": "disease.high_cholesterol", "high cholesterol": "disease.high_cholesterol",
  "gan nhiễm mỡ": "disease.fatty_liver", "gan nhiem mo": "disease.fatty_liver", "fatty liver": "disease.fatty_liver",
  "bệnh dạ dày": "disease.stomach", "đau dạ dày": "disease.stomach", "benh da day": "disease.stomach", "stomach": "disease.stomach", "stomach problems": "disease.stomach",
  "bệnh thận": "disease.kidney", "benh than": "disease.kidney", "kidney": "disease.kidney", "kidney disease": "disease.kidney",
  "khác": "disease.other", "khac": "disease.other", "other": "disease.other",
};

export const FOOD_EN = {
  "phở": "Pho", "phở bò": "Beef pho", "phở gà": "Chicken pho",
  "bún bò": "Beef noodle soup (bun bo)", "bún bò huế": "Hue beef noodle soup",
  "bún chả": "Grilled pork & noodles (bun cha)", "bún riêu": "Crab noodle soup (bun rieu)",
  "bún thịt nướng": "Grilled pork vermicelli", "bún": "Rice vermicelli",
  "cơm": "Rice", "cơm trắng": "Steamed white rice", "cơm tấm": "Broken rice with grilled pork",
  "cơm gà": "Chicken rice", "cơm sườn": "Rice with pork chop",
  "bánh mì": "Banh mi (Vietnamese baguette)", "bánh mì trứng": "Egg banh mi",
  "gỏi cuốn": "Fresh spring rolls", "chả giò": "Fried spring rolls", "nem rán": "Fried spring rolls",
  "hủ tiếu": "Hu tieu noodle soup", "mì": "Noodles", "mì xào": "Stir-fried noodles", "miến": "Glass noodles",
  "cháo": "Rice porridge", "cháo gà": "Chicken congee", "cháo trắng": "Plain rice porridge",
  "canh": "Soup", "canh chua": "Sour soup", "canh rau": "Vegetable soup",
  "rau luộc": "Boiled vegetables", "rau muống xào": "Stir-fried water spinach", "rau xào": "Stir-fried vegetables",
  "trứng": "Eggs", "trứng luộc": "Boiled eggs", "trứng chiên": "Fried eggs", "trứng ốp la": "Fried eggs (sunny-side up)",
  "ức gà": "Chicken breast", "thịt gà": "Chicken", "gà luộc": "Boiled chicken",
  "thịt bò": "Beef", "bò xào": "Stir-fried beef", "thịt heo": "Pork", "thịt lợn": "Pork",
  "cá": "Fish", "cá hấp": "Steamed fish", "cá kho": "Braised fish", "cá chiên": "Fried fish",
  "tôm": "Shrimp", "tôm hấp": "Steamed shrimp",
  "đậu hũ": "Tofu", "đậu phụ": "Tofu", "đậu hũ sốt cà": "Tofu in tomato sauce",
  "sữa chua": "Yogurt", "sữa chua không đường": "Unsweetened yogurt",
  "sinh tố": "Smoothie", "yến mạch": "Oatmeal", "salad": "Salad",
  "trái cây": "Fruit", "chuối": "Banana", "táo": "Apple", "cam": "Orange", "ổi": "Guava",
  "trà sữa": "Bubble milk tea", "nước ép": "Juice",
};
