...
--
-- SAU KHI chạy file này, BẮT BUỘC chạy tiếp migrations/fulltext_search.sql —
-- file đó thêm cột tsvector + chỉ mục GIN + hàm RPC tìm kiếm cho
-- admin_kb_chunks (Knowledge Base không dùng embedding ở bất kỳ đâu).
-- ============================================================
...
create table if not exists public.admin_pdfs (
  ...
  -- uploaded | extracting | chunking | saving | ready | error
  error_message text,
...
-- 3) Bảng chunks Knowledge Base admin (riêng, không đụng knowledge-base.json hiện tại).
--    Không lưu embedding ở đây — tìm kiếm bằng PostgreSQL Full Text Search
--    (tsvector + GIN + ts_rank), xem migrations/fulltext_search.sql (BẮT BUỘC
--    chạy sau file này để có cột tsv + chỉ mục GIN + hàm RPC tìm kiếm).
