...
function buildDish(text, grid, row, map) {
  ...
  const dish = {
    ...
    // File thực đơn dạng tự do không có cột giá — trả chuỗi rỗng để món đi ra
    // từ hai nhánh nhập (mẫu chuẩn / tự do) có cùng một hình dạng.
    price: '',
    base_grams: grams,
    calories: null,
    protein: null,
    ...
    ingredients: hasSpec
      ? spec.ingredients.map((i) => ({ name: i.name, grams: i.grams, unit: i.unit, price: '', tags: [] }))
      : [],
    source: 'excel_import',
    confidence: 'low',
...
export function parseIngredientLine(line) {
  ...
  if (lead) return { name: capitalize(lead[3].trim()), grams: toNumber(lead[1]), unit: normUnit(lead[2]), price: '', tags: [] };
  ...
  if (trail) return { name: capitalize(trail[1].trim()), grams: toNumber(trail[2]), unit: normUnit(trail[3] || 'g'), price: '', tags: [] };
