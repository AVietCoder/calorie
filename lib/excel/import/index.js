...
import { readProcessingInfo, readSourceHeader, readMealNotes, readIngredientDetails } from './read-meta.js';
 ...
 * ("Thực đơn mẫu"/) đặt tiêu đề ở hàng 5 vì phía trên còn tiêu đề sheet + ghi chú
...
export async function importMenuWorkbook(buffer, opts = {}) {
  ...
  for (const sheet of candidates) {
    ...
    /* Ghi chú + nguồn tra cứu nằm ở HAI SHEET KHÁC của cùng file. Gắn vào đây
       để tầng ghi DB chỉ nhận một cây `days` đã đầy đủ, không phải mở lại file. */
    const notes = readMealNotes(sheets);
    const ingDetails = readIngredientDetails(sheets);
    let reviewCount = 0;
    let enrichedIngredients = 0;

    for (const day of days) {
      for (const meal of day.meals) {
        const info = notes.get(`${day.day_index}:${meal.meal_type}`);
        if (info) {
          meal.note = info.note;
          meal.needs_review = info.needsReview;
          if (info.needsReview) reviewCount += 1;
        }

        /* Lượng MUA (khác lượng dùng) + tiền phải trả — chỉ có ở sheet
           "CHI TIẾT NGUYÊN LIỆU". */
        for (const dish of meal.dishes) {
          for (const ing of dish.ingredients || []) {
            const d = ingDetails.get(day.day_index, dish.name, ing.name);
            if (!d) continue;
            ing.buy_grams = d.buyQty;
            ing.buy_unit = d.buyUnit;
            ing.buy_price = d.buyPrice;
            if (d.group && !(ing.tags || []).length) ing.tags = [d.group];
            enrichedIngredients += 1;
          }
        }
      }
    }

    // Gộp hai nguồn xuất xứ: bài gốc (sheet nguồn) + khâu chuẩn hoá (THÔNG TIN
    // XỬ LÝ). Bài gốc để TRƯỚC vì đó mới là thứ người dùng cần đối chiếu.
    const meta = { ...(readSourceHeader(sheets) || {}), ...(readProcessingInfo(sheets) || {}) };
    const warnings = [];
    if (reviewCount > 0) {
      warnings.push(`${reviewCount} bữa được nguồn đánh dấu "CẦN KIỂM TRA" — số liệu dinh dưỡng chưa đầy đủ.`);
    }
    ...
    return {
      ...
      report: {
        ...
        notedMeals: notes.size,
        reviewMeals: reviewCount,
        enrichedIngredients,
        warnings,
        attempts,
      },
