 ...
 */
const PRICE_HEADERS = ['price', 'giá tiền', 'gia tien', 'gia_tien', 'giá', 'price_range', 'khoảng giá'];
...
export function parseLegacyFlat(grid) {
   ...
   */
  const priceCol = PRICE_HEADERS.map((h) => header.indexOf(h)).find((i) => i !== -1);
  const readPrice = (row) => (priceCol == null ? '' : String(row[priceCol] ?? '').trim());
