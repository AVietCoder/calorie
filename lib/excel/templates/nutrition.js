...
export function nutritionSheet(model) {
  ...
  const hasPriceRange = dishes.some((d) => String(d.price_range || '').trim() !== '');
  ...
  const columns = [
    ...
    ...(hasPrice ? [{ header: 'Giá tiền', role: 'text', width: 16 }] : []),
    ...(hasPriceRange ? [{ header: 'Khoảng giá', role: 'text', width: 26 }] : []),
