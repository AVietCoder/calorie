...
export async function uploadImage(buffer, filename = "food.jpg") {
  const m = mode();
  ...
  try {
    if (m === "signed") {
      ...
      const result = await new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { resource_type: "image", folder: IMAGE_FOLDER, use_filename: true, unique_filename: true, overwrite: false },
          (err, res) => (err ? reject(err) : resolve(res))
        );
        stream.end(buffer);
      });
    ...
    form.append("upload_preset", uploadPreset);
    form.append("folder", IMAGE_FOLDER);
