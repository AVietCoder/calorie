 ...
 * what happened.
 */
import { llm, LLM_MODEL, chatBody } from "../llm.js";
import {
  retrieveKnowledge,
...
export async function nutritionFromKnowledgeBase({ food, amount = "1 phần", lang = "vi", topK = 6 } = {}) {
  ...
  try {
    ...
    // Bỏ chat_template_kwargs/repetition_penalty: chúng nằm trong extra_body nên
    // chưa từng có hiệu lực (xem chatBody() ở lib/llm.js). frequency_penalty là
    // chuẩn OpenAI, đặt top-level thì cả hai nhà cung cấp đều đọc.
    const c = await llm.chat.completions.create(chatBody({
      model: LLM_MODEL,
