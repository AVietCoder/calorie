 ...
 *
 * Uses the schema defined in migrations/admin.sql + migrations/fulltext_search.sql:
 *   admin_pdfs       one row per uploaded PDF (status + counts)
 *   admin_kb_chunks  many rows per PDF (chunk_index, text, tsv tsvector)
 *
 * These tables are SEPARATE from the bundled disease-routed knowledge base
 * (see lib/rag/kb-base-store.js / table kb_base_chunks) — admin uploads are
 * a general knowledge layer that complements it.
 *
 ...
 *
 * NO EMBEDDINGS ANYWHERE. Retrieval is 100% PostgreSQL Full Text Search
 * (tsvector + GIN index + ts_rank, see migrations/fulltext_search.sql) via
 * the `search_admin_kb_chunks` RPC. Every function is defensive: on error it
 * returns a safe value and logs, so the chat flow is never broken.
 */
...
/** Insert chunk rows for a pdf (text-only — no embeddings). */
...
export async function insertChunks(pdfId, chunks) {
...
export async function listPdfs() {
  try {
    const { data, error } = await supabaseAdmin
      .from(PDFS)
      .select("id, file_name, file_size, cloudinary_url, cloudinary_public_id, storage_path, storage_bucket, status, error_message, chunk_count, uploaded_by_email, created_at")
...
/**
 * Full Text Search over the admin-uploaded PDF Knowledge Base via the
 * `search_admin_kb_chunks` Postgres RPC (tsvector + GIN + ts_rank). Used by
 * knowledge.js / kb-answer.js for every question — no bulk fetch, no
 * in-memory ranking, no embeddings.
 * @param {string} queryText
 * @param {number} [limit=10]
 * @returns {Promise<Array<{id, pdf_id, chunk_index, text, rank}>>}
 */
export async function searchAdminChunks(queryText, limit = 10) {
  const q = String(queryText || "").trim();
  if (!q) return [];
  try {
    const { data, error } = await supabaseAdmin.rpc("search_admin_kb_chunks", {
      query_text: q,
      match_count: limit,
    });
    if (error) {
      console.warn(`⚠️ [store] searchAdminChunks: ${error.message}`);
      return [];
    }
    return Array.isArray(data) ? data : [];
  } catch (err) {
    console.warn(`⚠️ [store] searchAdminChunks error: ${err.message}`);
    return [];
  }
}
