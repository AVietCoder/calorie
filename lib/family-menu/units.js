...
/** Bí danh đơn vị (gam→g, trái→quả…). Thêm đơn vị = sửa catalog JSON, không sửa code. */
const ALIASES = Object.fromEntries(
  Object.entries(catalog.unitAliases || {}).filter(([k]) => !k.startsWith('_'))
);

/** Mọi đơn vị hợp lệ, dài trước ngắn — thứ tự này quan trọng khi ghép regex. */
const ALL_UNITS = [
  ...new Set([
    ...Object.values(FAMILIES).flatMap((def) => Object.keys(def.units || {})),
    ...Object.keys(ALIASES),
  ]),
...
/**
 * Nguồn regex nhận diện đơn vị. Dùng `(?=…)` thay `\b` vì `\b` chỉ hiểu ASCII —
 * với "quả táo" thì `\b` đặt sai chỗ và bắt hụt.
 */
export const UNIT_PATTERN = ALL_UNITS.map(escapeRe).join('|');

/** 'gam' → 'g'. Trả null nếu không phải đơn vị nào ta biết. */
export function normalizeUnit(token) {
  const u = String(token || '').toLowerCase().trim();
  if (!u) return null;
  if (ALIASES[u]) return ALIASES[u];
  for (const def of Object.values(FAMILIES)) {
    if (def.units && u in def.units) return u;
  }
  return null;
}
...
export function unitFamily(unit) {
  const u = normalizeUnit(unit) || String(unit || 'g').toLowerCase().trim();
...
export default {
  unitFamily, toBase, convertForDisplay, roundForPurchase, formatQty,
  normalizeUnit, isKnownUnit, UNIT_PATTERN,
};
