    ...
    for (const m of sortedMeals) {
      ...
      for (const dish of m.plan_dishes || []) {
        ...
        const row = {
          ...
          name: dish.name,
          // Giá tiền: chuỗi nguyên văn, KHÔNG nhân theo số suất như dinh dưỡng —
          // nó là khoảng giá người nhập ghi cho món, không phải số để quy đổi.
          price: dish.price || '',
        ...
        // Nguyên liệu gắn thẳng vào món để sheet DINH DƯỠNG in ra được —
        // xuất rồi nhập lại thì không mất nguyên liệu.
        row.ingredients = (dish.plan_dish_ingredients || []).map((ing) => ({
          name: ing.name,
          grams: ing.grams != null ? Math.round(ing.grams * servingsFactor) : null,
          unit: ing.unit || 'g',
          price: ing.price || '',
        }));

        for (const ing of dish.plan_dish_ingredients || []) {
          const rawRow = { name: ing.name, grams: ing.grams, unit: ing.unit || 'g', price: ing.price || '' };
