  ...
  for (const templateDish of templateMeal?.menu_template_dishes || []) {
    ...
    const { data: planDish, error: pdErr } = await supabaseAdmin
      ...
      .insert({
        ...
        price_range: finalDish.price_range || '',
    ...
    const ingredients = planIngredientRows(planDish.id, finalDish, aggregated.totalFactor);
...
export async function generatePlan({ household, templateId, userId = null }) {
  ...
  if (!chosenTemplateId) {
    // Truyền userId để bước tự chọn thực đơn vẫn cân nhắc CẢ thực đơn người
    // dùng tự tạo, không chỉ thực đơn hệ thống — recommendTemplates mặc định
    // chỉ trả thực đơn hệ thống khi không biết người dùng là ai.
    const ranked = await recommendTemplates(household, members, { limit: 1, userId });
  ...
  /*
   * Ghi cả tuần theo LÔ, không phải từng dòng một.
   *
   * Bản cũ lồng ba vòng và chèn từng bản ghi: 7 plan_days → mỗi ngày ~4
   * plan_meals → mỗi bữa N plan_dishes → mỗi món 1 lô nguyên liệu + audit.
   * Cộng lại khoảng 170 round-trip Supabase nối đuôi nhau cho MỘT tuần; ở độ
   * trễ vài trăm ms mỗi lượt là hàng chục giây, và đó chính là chỗ Kitchen
   * gia đình/tổ chức bị chậm — không có lượt gọi AI nào ở đây cả.
   *
   * buildMealDish chỉ tính trong bộ nhớ (dishAllowedForHousehold và
   * findReplacement đều làm việc trên `pool` đã nạp sẵn, không chạm DB), nên
   * dựng xong TOÀN BỘ cây rồi mới ghi được — còn 5 lượt.
   *
   * Ghép id lại theo KHOÁ TỰ NHIÊN ở hai tầng đầu (day_index, meal_type) chứ
   * không dựa vào thứ tự trả về; riêng tầng món không có khoá tự nhiên nên
   * dùng thứ tự RETURNING của một lệnh INSERT nhiều dòng, vốn khớp thứ tự đầu vào.
   */
  const usableDays = template.days
    ...
    .map((d) => ({
      day: d,
      meals: (d.menu_template_meals || []).filter((m) => (m.menu_template_dishes || []).length > 0),
    }))
  ...
  if (usableDays.length) {
    ...
    const { data: dayRows, error: dErr } = await supabaseAdmin
      .from('plan_days')
      .insert(usableDays.map((x) => ({ plan_id: plan.id, day_index: x.day.day_index })))
      .select();
    if (dErr) throw dErr;
    const dayIdByIndex = new Map(dayRows.map((r) => [r.day_index, r.id]));

    /* 2 ─ bữa */
    const mealRowsIn = [];
    for (const { day, meals } of usableDays) {
      for (const m of meals) {
        mealRowsIn.push({ plan_day_id: dayIdByIndex.get(day.day_index), meal_type: m.meal_type });
      }
    }
    const { data: mealRows, error: mErr } = await supabaseAdmin
      .from('plan_meals').insert(mealRowsIn).select();
    if (mErr) throw mErr;
    const mealIdByKey = new Map(mealRows.map((r) => [`${r.plan_day_id}|${r.meal_type}`, r.id]));

    /* 3 ─ dựng món trong bộ nhớ (không chạm DB) */
    const dishRowsIn = [];
    const dishMeta = [];            // song song với dishRowsIn: nguyên liệu + audit
    for (const { day, meals } of usableDays) {
      const planDayId = dayIdByIndex.get(day.day_index);
      for (const templateMeal of meals) {
        const planMealId = mealIdByKey.get(`${planDayId}|${templateMeal.meal_type}`);
        for (const templateDish of templateMeal.menu_template_dishes || []) {
          const { finalDish, aggregated, audits } = await buildMealDish({
            templateDish, mealType: templateMeal.meal_type, members, mealsPerDay, pool,
          });
          dishRowsIn.push({
            plan_meal_id: planMealId,
            source_template_dish_id: finalDish.id,
            name: finalDish.name,
            // Giá tiền đi theo MÓN, không nhân theo số suất — đây là chuỗi
            // khoảng giá nguyên văn của người nhập, không phải số để tính.
            price: finalDish.price || '',
            price_range: finalDish.price_range || '',
            grams: aggregated.grams,
            calories: aggregated.calories,
          ...
          dishMeta.push({ finalDish, totalFactor: aggregated.totalFactor, audits });
        }
      }
    }

    /* 4 ─ món */
    if (dishRowsIn.length) {
      const { data: dishRows, error: dishErr } = await supabaseAdmin
        .from('plan_dishes').insert(dishRowsIn).select('id');
      if (dishErr) throw dishErr;

      /* 5 ─ nguyên liệu + audit, mỗi thứ một lô */
      const ingredientRows = [];
      const auditRows = [];
      dishRows.forEach((row, i) => {
        const meta = dishMeta[i];
        ingredientRows.push(...planIngredientRows(row.id, meta.finalDish, meta.totalFactor));
        for (const a of meta.audits) auditRows.push({ plan_id: plan.id, plan_dish_id: row.id, ...a });
      });

      if (ingredientRows.length) {
        const { error: ingErr } = await supabaseAdmin.from('plan_dish_ingredients').insert(ingredientRows);
        if (ingErr) throw ingErr;
      }
      if (auditRows.length) {
        const { error: aErr } = await supabaseAdmin.from('menu_adjustment_audit').insert(auditRows);
        if (aErr) throw aErr;
      }
    }
  }
...
export async function swapDish({ planDishId, replacementDishId, actor = 'user', reasonPrefix = 'Người dùng đổi món' }) {
  ...
  const newIngredients = planIngredientRows(planDishId, replacement, aggregated.totalFactor);
...
export async function buildShoppingList(planId, opts = {}) {
  ...
  const { data: rows, error } = await supabaseAdmin
    ...
    .select('name, grams, unit, price, plan_dishes!inner(plan_meals!inner(meal_type, plan_days!inner(plan_id, day_index)))')
...
export async function buildTemplateShoppingList(templateId, opts = {}) {
  const { data: rows, error } = await supabaseAdmin
    ...
    .select('name, grams, unit, price, menu_template_dishes!inner(menu_template_meals!inner(meal_type, menu_template_days!inner(template_id, day_index)))')
 ...
 */
/**
 * @param {number} totalFactor  TỔNG hệ số khẩu phần của cả hộ
 *   (aggregateDishForHousehold().totalFactor) — tức số "suất chuẩn" món này nấu
 *   cho. Đã bao gồm số người, KHÔNG nhân thêm memberCount nữa: bản cũ lấy factor
 *   của một thành viên rồi nhân số người, nên hộ có người ăn nhiều người ăn ít
 *   thì lượng nguyên liệu lệch hẳn so với calo đã cộng của chính món đó.
 */
export function planIngredientRows(planDishId, templateDish, totalFactor) {
  ...
  return src.map((ing) => ({
    ...
    unit: ing.unit ?? null,
    // Giá nguyên liệu KHÔNG nhân theo số suất — nó là đơn giá/khoảng giá nguyên
    // văn người nhập, phần nhân lên nằm ở cột Thành tiền của danh sách đi chợ.
    price: ing.price || '',
    tags: ing.tags || [],
  }));
}
