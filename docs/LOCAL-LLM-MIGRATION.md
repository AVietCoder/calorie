...
> ℹ️ Ghi chú: `scripts/ingest-knowledge.mjs` được nhắc tới bên dưới đã bị XOÁ
> trong đợt refactor bỏ embedding — Knowledge Base giờ dùng PostgreSQL Full
> Text Search, nạp dữ liệu nền bằng `scripts/seed-base-knowledge.mjs` thay
> thế. Xem [`docs/KNOWLEDGE-BASE-FULLTEXT-SEARCH.md`](KNOWLEDGE-BASE-FULLTEXT-SEARCH.md).
> Phần hướng dẫn chuyển sang vLLM cho model chat/vision bên dưới vẫn đúng.
