window.currentConvId = null;
  ...
  function _voiceSetRecordingUI(on) {
    const wrapper = document.getElementById('input-wrapper');
    const voiceBtn = document.getElementById('voice-btn');
    if (wrapper) wrapper.classList.toggle('is-recording', on);
  ...
  function initVoice() {
    ...
    voiceRecognition.onresult = (e) => {
      ...
      // Khi kết quả final: dừng ghi âm nhưng GIỮ text, không tự gửi
    ...
    voiceRecognition.onerror = (e) => {
      ...
      if (typeof showToast === 'function') showToast(msgs[e.error] || 'Lỗi: ' + e.error, 'error');
    };

    voiceRecognition.onend = () => { _stopRecognition(); };
  }

  // Dừng engine nhận giọng, thoát trạng thái recording — giữ text trong input
  function _stopRecognition() {
    isRecording = false;
    _voiceSetRecordingUI(false);
    try { voiceRecognition && voiceRecognition.stop(); } catch (_) {}
  }

  // Nút X: xóa text và thoát
  ...
  // Nút ✓: chỉ thoát recording, giữ nguyên text để user tự gửi
  function confirmVoice() {
    _stopRecognition();
    document.getElementById('user-input')?.focus();
  }

  // Nút mic: bắt đầu ghi âm (click khi đang ghi = confirmVoice)
  ...
  // Khi user đổi ngôn ngữ giữa chừng → dừng recording
  document.addEventListener('langchange', () => { if (isRecording) confirmVoice(); });
  // ──────────────────────────────────────────────────────────────
